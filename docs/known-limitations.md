...
- **现象**：`PathPoint::getKappa()` / `getKappaKinematic()` / `getV()` / `getDelta()` / `getA()` / `getDeltaDot()` 在对应派生量未设置时会抛出 `std::logic_error`，导致下游代码在忘记检查的情况下直接崩溃。
- **根因**：`PathPoint` 设计为 `Pose` + 一组"不一定存在"的派生量，用 `std::numeric_limits<double>::quiet_NaN()` 作为"未提供"哨兵，并通过 `has/get/set` 三件套强制调用方显式确认可用性。不同来源的 `PathPoint` 只会携带其来源天然具备的派生量：
  - `kappa`（对外统一的几何口径，不随方向签名）：`Path` 上由 `Path::finalize()` 的角度差分估计填充（逐点窄窗，内部中间量）；交付轨迹与 `Trajectory(Path)` 补全轨迹上由 `Trajectory::finalizeKappa()` 用加宽窗 `Trajectory::ComputeWindowKappa` 统一填充（2026-09-21 起），窗口不足的点保持未设置；
  - `kappa_kinematic`（运动学口径 tanδ/L）：仅供内部动力学相关消费，有 `delta` 的点由 `Trajectory::finalizeKappa()` 一并填充；与 `kappa` 的逐点差异即几何-运动学自洽性诊断信号（倒退段还存在方向符号约定差异）；
  - `v` / `delta`：来自 `NmpcSolver::ToPath()` / `pruneShortestSegment()` 对优化状态的回填；
  - `a` / `delta_dot`：来自上述 NMPC 方法对优化控制的回填，且每段最后一个点通常没有对应控制量，因此保持未设置；
  - 直接从 proto 反序列化或由 `Pose` 构造的 `PathPoint` 则全部派生量均未设置。
- **当前应对方式**：
  - 所有下游读取前必须先调用 `hasXxx()` 判断，例如 `PathToOcpConverter::interpolateAtArcLength` 在读取 `kappa` 时会对端点分别调用 `hasKappa()`，缺失时回退为 `0.0`；`Visualizer` 的 Path 详情画布在读取 `kappa` 前同样先 `hasKappa()`（轨迹对比画布自 2026-09-21 起改由 `Trajectory::ComputeWindowKappa` 按几何量统一计算，不再读取该字段）。
...
### NMPC data1 交付轨迹的几何伪影：几何 κ 尖峰与 xchk 分叉同源（与是否收敛无关）

- **现象**：`main.cpp` 曲率统计中，NMPC data1 的交付轨迹 `kappa[geom]` 出现远超物理上限的尖峰，而 `kappa[kin]` 全部在限内（×1.00）——**Release 收敛解**（"NMPC converged"）实测 max 0.4545（×2.64）、xchk（驱动段过滤后，过滤规则见 `docs/interfaces.md` 2026-09-21 `LogKappaStats` 条目）0.0942（×0.55，argmax i=117 v=-0.769 m/s、geom=0.267 vs kin=-0.172，行驶速度下几何与转向反向）；Debug 未收敛 last-iterate 上更极端（max 5.41 / p99 2.68，尖峰主体集中在蠕行段、被驱动段过滤按设计排除）。两种求解结局下尖峰位置同源（换挡/分段边界附近的同一段伪影几何）。
- **根因**：交付轨迹在换挡/分段边界附近的几何抖动（非物理可执行的朝向突变），运动学量（δ）并未跟踪该几何——这是真实几何-运动学不自洽，与"停驻区 δ 与几何解耦"的良性现象不同类（后者发生在 v≈0 且 geom 不形成持续尖峰）；与求解是否收敛无关（Release 收敛解同样携带），NMPC 收敛性本身是另一既有问题，此处仅登记其交付几何的判读口径。
- **当前应对方式**：以 `kappa[xchk]` 作为该类伪影的量化告警——iLQR 四数据集实测 ≤0.25× κ_max，NMPC data1（0.55×）显著超出；MINCO data6/3 读数 0.7× 的候选解释是低 ṡ 区 δ 经 ε_g 正则化失真（README §2 注脚与 MINCO cusp 条目），两类读数须按算法链路分别判读；不对输出做几何滤波（避免掩盖求解层问题）。
- **后续解决计划**：单独立项排查（候选：分段边界处的状态跳变、低速段 θ 变化与 v·tanδ/L 不同步——与既有"新暴露的现象"条目同址）；xchk 读数可作为该排查的验收指标之一。
- **来源**：2026-09-21 曲率双口径统一后的生产入口实测（main.cpp `LogKappaStats`，Release/Debug 双口径复测）。

### MINCO data1 预处理在 Debug 构建下失败（Release 正常）：L-BFGS 的构建类型数值差异

- **现象**：同一份代码、同一份配置，MINCO 路径在 data1 上——Release 构建收敛
  （10.967 m / 4 段，"MINCO converged"），Debug 构建预处理阶段即失败
  （"MINCO preprocessing failed (max endpoint error 0.106293 m)"，逐位确定性复现）。
- **根因**：预处理 L-BFGS 的迭代轨迹对浮点求值路径敏感（-O0 vs -O3/`-march=native`
  下的 FMA 与向量化合约差异），终点误差恰好压在收敛阈值（0.1 m）附近，构建类型
  成为压线案的裁决者。属边界数值案例，非某次提交引入的逻辑回归（stash 全部未提交
  改动后 Debug 下逐位一致复现）。
- **当前应对方式**：MINCO 路径的端到端验证与调参一律使用 Release 构建；Debug 构建
  仅用于单元测试（单测中的 MINCO 用例不依赖该压线场景）。
- **后续解决计划**：如需 Debug 下也可复现生产行为，单独立项评估——候选方向：
  预处理终点容差按实测分布重标定、L-BFGS 求值核的浮点合约固定（`-ffp-contract=off`
  对业务目标的影响需 benchmark）、或预处理初值/权重去压线化调参。
- **来源**：2026-09-21 曲率双口径工作中三数据集 xchk 验证时发现（人工指出
  "MINCO data1 之前是能够优化出来的"后定位）。
...
### build 根目录残留旧 protobuf 生成文件导致编译命中旧头（已处理）

- **现象**：proto 改用 `google.protobuf.DoubleValue` 后，`cmake --build
  build/Release` 编译报错 `request for member 'value' ... of non-class type
  'double'`——被编译的 `apa_post_process.pb.h` 仍是旧版（`optional double`
  生成的 getter 返回 `double`），尽管 `protobuf_generate` 已在新路径
  `build/Release/proto/apa_post_process.pb.h` 重新生成了含 `DoubleValue` 的新版。
- **根因**：`CMakeLists.txt` 的 `target_include_directories` 中
  `${CMAKE_CURRENT_BINARY_DIR}`（build 根）排在 `${CMAKE_CURRENT_BINARY_DIR}/proto`
  之前，而 build 根目录残留了历史配置输出的旧 `apa_post_process.pb.h/.cc`
  （孤儿文件，不在 ninja 目标内），include 优先命中了旧头。
- **当前应对方式**：删除 build 根目录的旧残留 `apa_post_process.pb.{h,cc}` 后
  重新编译即正常；正式生成文件以 `build/<config>/proto/` 为准。
- **来源**：2026-08-21 proto optional → DoubleValue wrapper 改造。

### iLQR 交付轨迹末点/停驻点 δ 可超幅值校验门（|v| 豁免漏检）

- **现象**：data7 交付轨迹末点（停驻 v=0.0009 m/s）δ=0.49397 rad，超物理
  上限 0.47728 达 +3.50%（等效 κ=tanδ/L ratio 1.0413），超生产校验门等效
  包络（δ 相对容差 2.1% → κ ratio ≈1.0247），但仍以「完全成功」交付。
- **根因**：后处理 δ 幅值门对 |v|<post_v_dwell（0.05 m/s）的状态直接豁免
  （src/core/iLQR/ilqr_post_stage.cpp:659）——设计意图是停驻点转角不参与
  瞬时运动，但交付轨迹仍携带该值：下游直接使用 δ 或由 δ 换算 κ 时会读到
  超机械上限的指令（真实执行器只能饱和）。运动中（|v|≥0.05 m/s）的点四
  数据集均在包络内（最大 +1.79%，data3）。
- **工程后果评估（2026-09-01，人工裁决 + 实测，定性降级）**：末点 δ 超限
  在泊车场景下无实际运动学后果——四数据集 × 开/关虚拟控制共 8 次运行，
  末点位置误差 ≤0.007 m（门 0.05 m）、航向误差 ≤0.17°（门 1.5°），全部
  达标；末点停驻后无后续运动指令，末样本的 δ 不驱动任何位移（实测末段
  爬行点：v=0.015~0.045 m/s 时 δ 尚在限内，超限只出现在 v≈0 的最后
  1~2 个样本）。据此定性由「超生产校验门的合规缺陷」降级为「交付物
  整洁性缺陷」（携带执行器不可达但不会被轨迹跟踪消耗的 δ 值）。关虚拟
  控制后该现象仍在且 data3 末点 δ=-0.50037（+4.84%）更差，证实其与
  虚拟控制无关（终端等式约束只管 (x,y,θ) 不管 δ + 幅值门速度豁免）。
- **当前应对方式**：无（四数据集双口径核验曾用一次性诊断工具完成，
  结论已固化到本条目，工具本身按一次性使用原则已删除；复现路径为
  生产场景链路 + 逐点 tanδ/L 与 Δθ/Δs 双口径统计）。
- **后续解决计划**：低优先级整洁性清理——交付前把停驻点（|v|<
  post_v_dwell）的 δ 钳制到 [-δ_max, δ_max]（同时同步 κ=tanδ/L 字段），
  避免下游无速度门控的消费者读到不可执行值；待 M013 裁决是否顺带实施。
- **来源**：2026-09-01 曲率双口径独立核验（milestone-013 review-log
  战术改动 #12）。

### iLQR 交付轨迹在换挡过渡段几何-状态不自洽（单步残差被 3° 运动学门放行）

- **现象**：交付轨迹在 v 过零的 cusp 过渡段，每步实际 Δθ 比 v·tanδ/L·dt
  系统性单边偏大 ~0.004 rad（≈0.25°），20 余步单向累计 ≈5.7°/0.21 m
  （data6 实测），加宽窗（半窗 ≥0.1 m）几何曲率达 0.48 1/m（物理上限
  0.17239 的约 2.8 倍）；远离过渡段的匀速段两种口径吻合（偏差 <3%）。
  单步残差远在运动学门（3°）内，交付门不拦。
- **根因（2026-09-01 代码走查 + 逆向动力学定量验证，已定位）**：
  **ALTRO 虚拟控制稳态残留**。① 内层 MS-iLQR 默认开启虚拟控制增广，
  rollout/名义建立交付的是增广动力学 x⁺=f(x,u)+w（ms_ilqr.cpp
  `nonlinearRollout`/`setNominalTrajectory`），w 为逐状态步的 7 维附加
  增量，软代价 ½·R_inf‖w‖²·dt（R_inf=1e4 固定、无递增调度）；② 编排层
  明确不要求 w 趋零（apa_ilqr_solver.cpp `virtual_ok` 硬编码 true，
  注释「w 稳态随对偶残差缩放」），外层可行性只看打靶缺陷（≤1e-3、
  仅存在于打靶节点）与幅值/终点，w 不在任何收敛/交付判据内；③ 初值在
  cusp 处动力学不可行（v_init 恒 ±v_nominal=0.5 m/s 符号跳变、无减速
  过渡，ilqr_reference_builder.cpp 初值提取），首轮 w 反解初始化即把
  全部违反吸入，cusp 过渡段的跟踪/门控代价使 w_θ 以稳态残留留存
  （每步买 0.1°~0.3° 朝向转动的成本仅 ~1e-2，远低于不跟踪的罚）。
  定量对拍：由交付轨迹经逆向动力学重建等效 w（a/ω 差分反解控制量后
  逐级预测 v/δ/θ/x/y），cusp 过渡段 |w_θ| 持续单边 0.002~0.005 rad/步
  （干净段背景 0.0005~0.0009），与观测 Δθ 超额逐项吻合；打靶缺陷
  （≤1e-3、仅打靶节点、25 步一遇）在量级与形态上都无法解释该持续偏置。
  附带观察：cusp 窗内另有 |w_v|~0.016、|w_δ|~0.005 尖峰（驻留窗重定时
  叠加）。此前「后处理接缝窗改写未重积分」的假设被该证据取代。
- **决定性对照（2026-09-01，`use_virtual_control=false` 四数据集重测）**：
  关 w 后逐对航向残差 max 降至 0.0006 rad（打靶缺陷量级），口径一/口径二
  统计逐项重合，加宽窗几何 κ 最大比值全部回落到 ≤1.01（data6 0.99）——
  cusp 区几何超限归零，坐实 w 残留是唯一来源。注意修正一个易错表述：
  原始逐对差商的爆炸并非「密集采样固有波动」，而是 w_θ 残余被 ds→0
  放大；w≈0 时 Δθ 与 Δs 同步趋零，差商有界。但关 w 的代价是四数据集
  全部退化为降级输出（阶段二不可用）、长度全面变长（最大 +39%，
  data6 12.71→17.65 m）、data7 段数 2→4——虚拟控制承担着「从不可行
  初值起步、升阶段二收敛」的结构职能，正确方向是管住残留而非关闭。
- **当前应对方式（2026-09-02 人工裁决，已生效）**：**默认关闭虚拟控制**
  （`iLQRConfig::inner_use_virtual_control` 与 `data/ilqr_config.json` 同步
  置 false）——人工权衡后接受关 w 的代价（四数据集全部降为阶段一降级
  输出、长度变长、data7 段数 2→4）换取交付轨迹的纯动力学一致性（关 w
  后逐对航向残差 ≤0.0006 rad、加宽窗几何 κ 全部 ≤1.01× 上限）。开启仅
  作实验对照。几何口径判读须知：开 w 时原始逐对差商在 cusp 邻域被
  ds→0 放大到 1.9~10.1 1/m（放大的是 w 残余而非采样噪声），该口径
  原始形态无判决价值，必须用加宽窗形态判读。
- **后续解决计划**（仅当未来重新启用虚拟控制时适用，互斥/可组合）：① 把 ‖w‖∞（或按分量
  w_θ/w_pos）纳入外层可行性判据与 post-stage 校验门，使 w 残留过大的
  解不可交付；② R_inf 随 AL 外层轮次递增，压低 w 稳态量级；③ 参考构建
  侧消化：cusp 邻域朝向剖面按 v 剖面重定时，让参考本身动力学可行，
  不逼求解器用 w；④ 运动学门对低速段收紧（3°/对在 |v|→0 时等效 κ
  无界）。注意勿与已证伪的「δ 投影+级联重积分热启动」变体混淆（那条
  红线针对打靶缺陷修复，不适用于 w 的处理）。
- **来源**：2026-09-01 曲率双口径独立核验与根因走查（milestone-013
  review-log 战术改动 #12）。

### 曲率有两套口径：`PathPoint::kappa`（逐点窄窗）与绘图/统计口径（加宽窗）

- **背景**：仓库里长期并存两种"曲率"，此前由不同消费者各取所需，结果是同一张对比图上
  三条曲线的口径互不相同——iLQR/NMPC 画的是求解器状态 $\tan\delta/L$、MINCO 画的是逐点
  差商 Δθ/Δs、原始路径画的是 σ·Δθ/Δs；换挡点邻域的差商尖峰被当成"超限"，而真正的
  几何-运动学不自洽（如虚拟控制残余）在状态量曲线上反而完全隐身。
- **两套口径与分工（2026-09-21 起）**：
  1. `PathPoint::kappa`：`Path::finalize()` 的角度差分，**逐点窄窗**（内部点中心差分、
     段端点单侧差分，窗口即相邻采样点间距）。它是算法内部消费（`PathToOcpConverter` 的
     δ 初值、`ComputeTimeProfile` 的侧向加速度限速等）与数据契约的一部分，保持原样；
  2. 绘图/统计口径：`src/util/trajectory.h` 的 `Trajectory::ComputeWindowKappa`（2026-09-21 初版位于
     `src/util/kappa_metrics.h`，同日并入 `trajectory.h`）——沿弧长以
     **0.1m 半窗**计算（双侧中心差分，段端单侧差分），只依赖交付几何量 (x, y, θ)，
     **不依赖求解器状态量 δ**，因此三种算法与原始路径天然可比；方向段按 v 符号游程切分、
     换挡点不跨越；`Visualizer` 的轨迹对比画布与 `main.cpp` 的 `LogKappaStats` 共用该实现，
     图上另叠加 ±κ_max 参考线（参考线纳入 band 量程）供直接判读超限。
- **半窗取 0.1m 的依据**：iLQR 虚拟控制残余诊断中，"加宽窗（半窗 ≥0.1m）"已被验证能把
  ds→0 的放大效应压掉并保住判决力（原始逐对差商 1.9~10.1 /m 无判读价值，加宽窗 0.48
  = 2.8× 上限可判读）；它同时天然抑制换挡折返处的差商尖峰——统一口径后 MINCO 四数据集
  几何 κ 收敛到 1.00~1.04 倍上限（此前 data1 逐点差商达 0.25、data6 曾达 0.96）。
- **判读须知**：几何 κ 取**几何符号**（倒车段与 tanδ/L 相反），判超限只看幅值；加宽窗会
  把单点窄窗尖峰按窗宽摊薄，故它回答的是"这段路面对转向的要求"而非"某一点的瞬时需求"；
  需要逐点形态时应直接看 `PathPoint::kappa`。
- **新暴露的现象（待定位）**：NMPC `data1` 的几何 κ 最大值 0.4545（2.64× 上限，7/184 点
  超限），而同一条轨迹的 tanδ/L 仅 1.00×。两者分叉说明该处交付几何与运动学状态不一致
  （候选原因：分段边界处的状态跳变，或低速段 θ 变化与 v·tanδ/L 不同步），此前用状态量
  口径完全看不到。建议后续用"独立积分器重放 + 段边界 θ 连续性核对"定位，勿直接归因于
  某一个算法。
- **来源**：2026-09-21 曲率口径统一（绘图与统计改用加宽窗几何曲率，新增
  `Trajectory::ComputeWindowKappa`（初版在 `src/util/kappa_metrics.h/.cpp`，同日并入
  `src/util/trajectory.h/.cpp`）与 10 个单元测试）。

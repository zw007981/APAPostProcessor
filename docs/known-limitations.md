...
- **现象**：`src/core/ALM/alm_preprocessor.cpp` 与 `src/core/ALM/alm_solver.cpp` 的 `evaluateCostAndGradient()` 中，"物理约束惩罚"梯形积分循环（约 60 行）与"段时长平衡约束"循环（约 25 行）逐字重复（仅代价累加变量名 `cost`/`j_s_prime` 不同）；`computeSimpsonNodeData()` 的核心离散逻辑与 `SimpsonUnitWeights()` 也高度重复（`SimpsonUnitWeights` 两文件完全一致）；角度归一化（wrap 到 $(-\pi,\pi]$）存在三处语义相同的独立实现：`AlmSolver::NormalizeAngle`、`src/core/collision_check.h` 的自由函数 `NormalizeAngle`、`src/core/DDP/ddp_reference_builder.h` 的自由函数 `WrapAngle`（DDP 模块后新增，NMPC 路径另有 `third_party/StcSQP::math_util::NormalizeAngle`）。
...
- **来源**：一次全局质量校验评审（📝 建议，不采纳本轮处理并登记为技术债）；后续一次全局质量校验复核（audit-001）确认三处角度归一化实现仍建议维持现状（提取公共工具的收益低于跨模块新增依赖的成本），仅补充登记 DDP 模块新增的第三处同语义实现 `WrapAngle` 以保持记录完整性。
...
### DDP 模块合入提交中 `src/core/DDP/esdf_constraint.h` 为空文件（HEAD 不可编译，已重建）

- **现象**：DDP 求解器合入提交（`58187e2`）中 `esdf_constraint.h` 为空 blob（e69de29），但 `esdf_constraint.cpp`/`ddp_cost.h`/多个测试与 bench 均包含该头并引用 `DdpEsdfConstraint`/`DdpEsdfConstraintConfig`/`DdpEsdfPoseCost`/`DdpEsdfCircleConstraint`/`DdpStateHessian`——工作区从该提交检出后全量构建必然失败。
- **根因**：大提交合入时头文件内容未被一并提交（`tool/tune_ddp.cpp` 同为该提交引入的空占位文件）。
- **当前应对方式**：按 .cpp 实现、单元测试用法与 interfaces.md 冻结清单逐字段重建头文件（`DdpStateHessian` 定长类型、配置结构体默认值与 `data/ddp_config.json` 一致、构造函数第三参数带默认值、`evaluateCircle` protected 供白盒对拍），670/670 单测通过验证重建语义正确。
- **后续解决计划**：无；该事件本身作为"大提交必须全量构建验证"的流程教训留存。
- **来源**：M009 开发进场时的构建阻断修复。

### `AlmManeuverSegmenter({})` 花括号二义调用导致 HEAD 测试编译失败（已修复）

- **现象**：`test/alm_solver.t.cpp`/`test/alm_preprocessor.t.cpp` 共 4 处 `AlmManeuverSegmenter({})` 在当前编译器（GCC，C++17）下报 `call of overloaded ... is ambiguous`——`{}` 既可匹配 `explicit AlmManeuverSegmenter(AlmManeuverSegmenterConfig config = {})` 又可匹配拷贝/移动构造。
- **当前应对方式**：改为 `AlmManeuverSegmenter()`（默认实参构造，语义逐位不变）。
...
- **现象**：DDP.md 2.6 校验清单⑥假设"优化器把转向摆动完整排入静止窗（窗口按 T_resteer 定宽）、窗端 ω≈0"；真实数据集大 Δδ 接缝（0.5~0.9 rad）实测窗端 |ω| 普遍 0.34~0.50（≈ω_max），摆动以低速滚动出窗完成。
- **根因**：平滑代价 ∫η²dt∝Δδ²/T³ 决定优化器总偏好把摆动摊出窗口；窗口加宽（κ_pad 定宽 1.2/1.5/1.8 三档实验）对该偏好无影响，反而降低阶段二收敛鲁棒性并引入运动学残差（实验已干净回退）。物理可执行性由校验④的 |ω|≤ω_max、|η|≤η_max 硬限独立保证（残余摆角 ≤0.13 rad 在蠕行出窗段完成，与 DDP.md 2.6 的 v_roll 可选变体同一物理图像）。
...
- **后续解决计划**：若底盘规范要求严格静止转向，实现 DDP.md 2.6 的 v_roll 变体（窗口速度帽放宽为 |v|≤v_roll）或在窗端施加 ω=0 的 AL 等式门控，而非收紧本容差。
- **来源**：M009 四数据集调参（κ_pad 窗宽实验证伪 + 接缝级报告量测）。

### ESDF 罚刚度上限：weight_safe≥200 或 margin_safe≥0.03 会让贴障初值的首轮内层直接溢出（DDP 路径调参雷区）

- **现象**：DDP 路径下 weight_safe=200/400 或 margin_safe=0.03/0.05 均使 data3 阶段一在第 0 轮外层即 INNER_SOLVER_FAILED（λ=0、首轮弱终端罚下 ESDF 罚梯度主导，GN 方向过冲、线搜索全面拒绝）。
- **当前应对方式**：weight_safe=100/margin_safe=0.02 是阶段一可存活的上限并保持默认；阶段一收敛解的平衡侵入（data3 0.016 m、data7 0.009 m）已满足 0.02 m 碰撞质量门，无需上调。ALM 侧 weight_safe=600 的标定不可移植（求解器结构不同）。
...
- **来源**：M010 四断点审计（`apa_audit_ddp_post` 的 [AUDIT-KINV] argmax 定位）。
...
- **当前应对方式**：裁剪改为以前一**窗口右边界+1** 为左端下界（右端仍不跨下一接缝点），窗口严格互不重叠且恒包含自身接缝（前一窗口右端 ≤ 本接缝−1 保证下界不超接缝）；回归测试 `DdpDwellInsertTest.ClippedWindowsOfCloseSeamsStayMonotonic` 钉住该语义。生产四数据集逐位无变化。
- **来源**：M010 Round 1 评审建议（R1-06，多接缝时间戳单调性测试）驱动发现，先写测试复现重叠再修复。

### DDP 换挡代理 ℓ_shift 与全局/局部退火调度（M011 已证伪方向，勿重试）
...
- **现象**：M011 对「maneuver 数显著降低」做方案层优化时，以下方向经四数据集全部实测证伪（逐组数据见 docs/milestones/milestone-011/review-log.md 实验流水账与 `build/log/tune_m011_*.txt` 留档）：
  1. **换挡代理代价 ℓ_shift**（DDP.md 2.3 节可选工程变体）：精确 Hessian 与 PSD 投影两种曲率形态 × w_g∈{0.1,0.5,2,10} 全部出局——w_g=0.1 即令 data3 阶段一发散（终点航向误差 90°）、data6 穿出障碍（碰撞 0.63 m），cusp 处 |v| 峰值不响应。机理：该项对「保留的换挡」与「待融的换挡」无差别惩罚，且 v≈0 区与 tanδ 奇异区相互作用；PSD 投影排除负曲率嫌疑后仍失稳，证伪在机制层面成立。`weight_shift` 保持默认 0。
...
- **当前应对方式**：接受基线融化水平；`AlOuterLoop`/`DdpCostInput` 保留了候选掩码、β 退火、退火保持轮数等默认关闭的调度机制（含单元测试），供后续结构性方案复用。
...
- **实测规模**（单次 data6 运行 `build/log/log_20260731162332.2.log`）：
  ...
  合法解根本不该接近**。
- **代码注释的设计判断是反的**：`DdpEsdfConstraint` 写「越界/非有限查询时为
  零（**不引入虚假恢复方向**）」——对优化器而言，零梯度不是「不引入虚假方向」，
  而是**造了一个没有出口的平坦陷阱**。`docs/interfaces.md` 中
  `AlmEsdfPenalty`/`DdpEsdfConstraint` 的契约描述把该行为写成了正式约定，
  **该约定本身即缺陷**。
- **设计文档缺口**：`docs/DDP.md` 中「边界外/越界/可行域有界/inMap」**零命中**
  ——2.4 节把 ESDF 当作无条件可用的距离场，从未定义过定义域之外的语义。
- **跨模块**：ALM 路径（`alm_esdf_penalty.cpp:59`）与
  `ddp_reference_builder.cpp:118`（`getDist`）共用同一查询，**ALM 同样带着这个
  缺陷**，只是它的解从未跑出地图所以未暴露。修复时必须全量回归 ALM。
...
- **✅ 已修复（2026-07-31，Dev Agent 落地 L8.1~L8.5）**：图外按实心障碍
  ...
  契约细节见 docs/DDP.md 2.4 节「ESDF 定义域之外的语义」。**修复后
  ...
  **仍然成立**（原碰撞数字含图外误报成分但判定不变）；「data6 无解」
  ...
  ALM 四数据集全量回归通过（maneuver 逐位不变、长度 −0.7%~+1.9%）。
  逐项证据见 review-log Round 4 回应。

### DDP L7 层机制（M011 Round 3 结论，勿重试/采纳项各一）
...
3. **异构回退 ALM（L7.4）**：**人工裁决不批准**——DDP 路径保持纯
   DDP 语义，失败回退原始 A*，不输出 ALM 解。本项为产品口径决策，
   后续不得由 Agent 自行重提。
...
### DDP L6 层机制（M011 Round 2 已全部证伪/留档，勿重试）
...
> tune_l64|l63a|l62b2.log。全部机制代码与单测保留、默认关闭。
...
- **两个结构性问题**：① `sample_dist`/`dt` 都是按空间分辨率与积分
  ...
  ② $T$ 不是决策变量，而 **ALM 侧 MINCO 的段时长 $T$ 本身就是决策
...
### DDP 求解器数值层调度（M011 已证伪方向，勿重试）

- **证伪清单**（逐项实测证据同上）：
  1. **完整二阶 DDP**（`APA_DDP_FULL_HESSIAN` 编译开关）：Li 2023 的局部二次收敛收益在本问题上被病态化代价抵消——四数据集中三个阶段一直接不收敛，data7 收敛但 6→4 变劣。
...
- **幅值组独立 μ 上限（`amplitude_mu_max`）为零回归可用机制**：健康数据集逐位不变（其幅值组从未触及上限）；data6 的「终端/幅值 μ 指数攀升死亡螺旋」被打破（终端收敛到 0.006 m/0.00004°），但打靶缺陷随之不愈合（µ_m 钉住下无力支付缺陷修复），仍无合法输出——默认不启用，机制保留。
- **来源**：M011 L3 层实验。
...
### DDP 曲率正则与 κ 门（L5 层已证伪方向与口径事实，勿重试）
...
- **曲率正则 ℓ_κ=w(tanδ/L)² 按 spec 三档 {22,67,225} 全部碰撞出局**：
  ...
  本身**——机制保留默认关闭（`weight_curvature=0`），不建议后续再试
  同族形态（含 δ² 类罚项）。
- **κ 门的口径事实（测量结论，非可调参数）**：
  ...
  2. AL 不等式的平衡残余使输出可能落在幅值上限之上，但**最终标定下实测
     ...
     只能取车辆真值、不得再内缩**。
     - **验收包络裁决（M011 Round 2）**：`tool/tune_ddp.cpp` 中期采用的
...
### DDP 幅值上限曾长期与车辆物理参数脱钩（已修复：clampToVehicleParams）

- **现象**：DDP 链路的 δ/ω 幅值上限曾以 JSON 硬编码值运行
  ...
  残差不查绝对上限、`tool/tune_ddp` 从未量测曲率（对照
  `tool/tune_alm.cpp` 一直在报 `kappa_ratio`）。
- **根因**：DDP 链路从零处引用 `VehicleParams::max_steer_angle`/
  `max_steer_rate`（对照：ALM 链路经 `DeriveKinematicsConfig` 显式
  同源）；`synchronizeAmplitudeBounds()` 把 JSON 值单向传播为唯一权威
  来源，没有任何车辆参数收口。
- **当前应对方式**：`DdpConfig::clampToVehicleParams` 已落地（δ_max/
  ω_max/a_max 只准收紧到车辆真值并注入轴距），`optimizeDdp` 单一
  收口点先钳制再同步；结构体默认值与 `data/ddp_config.json` 已改为
  车辆真值；`tool/tune_ddp.cpp` 已接入 `max_kappa`/`kappa_p95`/
  `kappa_ratio`/`max_omega` 量测与门检；`audit_ddp_post` 的 `kappa_cap`
  改用车辆真值。历史四数据集数字（M009/M010 各表）均为假上限口径，
  真值口径见 docs/DDP.md 3.8 节。
...
- **根本预防**：任何进入代价/约束/校验的物理量边界，其唯一权威来源必须是
  ...
  调参工具必须量测并上报该量的实际取值与上限比值——DDP 侧这个问题拖了
  三个 Milestone 才暴露，直接原因就是 `tool/tune_ddp.cpp` 从未量测曲率，
  而 `tool/tune_alm.cpp` 一直在报 `kappa_ratio`。

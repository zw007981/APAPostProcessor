...
`NmpcSolver`。`main.cpp` 生产入口已改为只接入 MINCO 路径（见 3.7 节），不再直接消费
NMPC 链路的任何产出，编译期依赖图中不再保留 `nmpc --> main`/`preprocess --> main` 边，
NMPC 路径继续由 `NMPCPlanningScene`、单元测试与 benchmark 独立调用。

### 3.7 MINCO/MINCO/PHR-ALM 优化领域模型

作为与 3.4 节 NMPC 并列的第二条后处理算法路径，`src/core/MINCO/` 面向同一 APA 场景实现一套
...
[docs/MINCO.md](MINCO.md)（第一章为原论文理论框架，第二章为本仓库自行车模型下的 APA 重新推导），本节
只做架构层面的领域模型概述。

```mermaid
flowchart LR
    Path -->|MincoManeuverSegmenter| Init[初始 M 段估计 p_w0/s_m/theta_m/tau_m]
    Init --> Preproc[MincoPreprocessor 粗优化 Jpre]
    Preproc -->|MincoTrajectory 初值| Solver[MincoSolver 内层 L-BFGS]
    ESDFMap -->|MincoEsdfPenalty| Solver
    VehicleFootprintModel -->|MincoEsdfPenalty 外圆集合| Solver
...
- `MincoTrajectory`：$\theta_i(t)/s_i(t)$ 多项式段表示，封装 $K(T)c=b$ 装配、块 Thomas 求解、
  时间重参数化 $\tau \leftrightarrow T$ 双射以及终点弧长 $s_f$ 的伴随梯度（对应 [docs/MINCO.md](MINCO.md) 1.2 节）。
- `BicycleKinematicsExtractor`：从 `MincoTrajectory` 的 $\theta,s$ 各阶导数解析阿克曼状态/控制量
  （$v,a,\delta,\dot\delta$）与对应的防奇异物理约束惩罚（对应 2.2/2.3 节）。
- `MincoManeuverSegmenter`：复用 `Path`/`Maneuver`，实现换挡打断（宏观段）与空间等距降采样（微观段），
  产出初始 $M$ 段估计（对应 2.1 节）。
- `MincoEsdfPenalty`：复用 `ESDFMap`/`VehicleFootprintModel` 外圆集合，实现 `margin_safe`/`margin_comf`
  双重松弛罚函数与梯度反传（对应 2.4 节）。
- `MincoPreprocessor`：两阶段优化流程的第一阶段，松收敛阈值下把初值拉近前端路径（对应 1.2/2.1 节
  “两阶段优化流程”）。
- `MincoSolver`：与 `NmpcSolver` 并列的求解器编排入口，内层 L-BFGS（复用 `third_party/LBFGSpp`，
...
该路径已全部落地并接入生产后处理器：`PostProcessor::optimizeMinco()` 把上述组件编排为
`Path → MincoManeuverSegmenter → MincoPreprocessor → MincoSolver → MincoManeuverMelter` 完整链路，
与既有 NMPC 路径并列、互不影响（运动学配置由 `VehicleParams` 统一派生，碰撞质量门 0.02 m
两条路径共享）。四数据集端到端验收与 NMPC 的对比结果见 [docs/MINCO.md](MINCO.md) 第三章。
`main.cpp` 生产入口只构造并执行 `MINCOPlanningScene`，用 `Visualizer` 生成 **MINCO 预处理粗优化
轨迹（优化前）与 MINCO 主优化 + 机动融化后轨迹（优化后）** 的对比图：两条轨迹由同一套离散化
工具（`SampleMincoTrajectory`，`src/core/MINCO/minco_trajectory_sampler.h`）产出，是同一条 θ-s
...
复核发现 `MincoManeuverMelter` 的 `PIVOT` 处理与自行车模型自相矛盾（详见
[docs/known-limitations.md](known-limitations.md) 对应条目与 [docs/MINCO.md](MINCO.md) 3.4 节），
该问题已修复；`main.cpp` 此前需要运行一次完整 NMPC 求解才能获取其预处理管线
产出作为对比基线，现已改为直接对比 MINCO 自身"预处理粗优化轨迹"与"主优化+机动
融化后轨迹"（离散化能力从 `MincoManeuverMelter` 拆出为独立工具 `SampleMincoTrajectory`），
`main.cpp` 不再依赖 `NMPCPlanningScene`，MINCO 成为真正独立的优化器；
...
最后完成四数据集调参：MINCO 采样轨迹补齐时间戳使运动学门对其生效、
...
（11.77/6.98/13.87/27.80 m），过程与批次数据见 [docs/MINCO.md](MINCO.md) 第四章。

### 3.8 iLQR（MS-iLQR + AL）优化领域模型（第三条算法路径，求解链路已落地、效果攻坚进行中）

与 3.4 节 NMPC、3.7 节 MINCO 并列的第三条后处理算法路径，完整数学推导与工程约定见
[docs/iLQR.md](iLQR.md)（理论基础：Tassa 2014 Box-DDP、Howell 2019 ALTRO、Li 2023
...
flowchart LR
    ...
    P --> S1["阶段一：全局软化 iLQR<br/>跃度主导 + 退火跟踪 + AL 约束<br/>（融化无效 maneuver）"]
    S1 --> PR["后处理<br/>符号游程分析 + 拓扑修剪<br/>（复用 TopologyCleaner）"]
    PR --> S2["阶段二：门控精化 iLQR<br/>符号门控 + 接缝静止窗 + 热启动重解"]
    S2 --> D["驻留插入 + 通过性校验"]
    D -->|通过| O["输出轨迹"]
...
验收结果为 **2 收敛 + 2 回退**（详见 [docs/iLQR.md](iLQR.md) 第 3 章），一半场景零收益。
...
- **010（先做）**：审计并重构后处理与通过性校验口径。已识别的结构性嫌疑包括——修剪
  PIVOT 判据借用了 `topology_cleaner` 的**前轮转角差 Δδ** 语义（MINCO 侧用的是**朝向差
  Δθ**，物理含义完全不同）、剔除弧长阈值 0.05 m 与 iLQR 固定 dt 网格量纲不匹配、
  校验清单在两条路径间严重不对称（MINCO 3 项 vs iLQR 12 项且任一失败即整体回退）、
...
- **011（后做）**：在正确的口径上做方案层与参数层的系统优化，目标是「合法前提下
  ...
  iLQR、merit 棘轮松绑）→ 结构性方案（多同伦候选并行择优）。

**规划中的组件划分**（`src/core/iLQR/`，接口规划见 [docs/interfaces.md](interfaces.md)，
Milestone 拆分见 [docs/milestones.md](milestones.md)）：

- 预处理/参考构建：等弧长重采样到 0.05 m、cusp 检测、差分初值提取、打靶节点布设（对应 iLQR.md 2.1 节）。
- 七维自行车动力学：半隐式 Euler + 中点朝向角离散化、解析雅可比 $A_k/B_k$
  （可选完整二阶张量编译期开关已证伪并于 2026-08-04 删除，对应 2.2/2.5 节）。
- box-QP 求解器：投影牛顿 + 活动集热启动，返回自由维 Hessian 分解（对应 1.2/2.4 节）。
- 代价/约束求值层：平滑主项、退火跟踪、状态幅值平方 AL 不等式、ESDF 双 margin
  惩罚（复用 `ESDFMap`/`VehicleFootprintModel` 外圆集合与 discretize-then-differentiate 约定）、
  终点 AL 等式（对应 2.3/2.4 节；可选换挡代理已证伪并于 2026-08-04 删除）。
- MS-iLQR 内层：缺陷感知 Riccati 回推（右端索引约定）、LM 正则化调度、线性 rollout 缓存
  $EC_1/EC_2$ + 非线性 rollout 线搜索（对应 1.4/2.5 节；可选段间惩罚已于 2026-08-04 删除）。
- AL 外层与求解编排：自适应 $\mu^0$、门控增长、跟踪权重退火、阶段一/阶段二统一入口（对应 2.5 节）。
- 后处理：符号游程分析（滞回）、拓扑修剪（复用 `util::topology_cleaner`，红线同 MINCO）、逐接缝
  $T_{resteer}$ 驻留插入、校验清单与回退（对应 2.6 节）。
- 场景接入（已落地）：`PostProcessor::optimizeiLQR()` 编排入口 + `ILQRPlanningScene` + `data/ilqr_config.json`
  （`"algorithm"` 路由字段，沿用 3.7 节确立的「每算法一个配置详情 JSON」约定）。

性能画像（iLQR.md 2.2 节复杂度注记）：纯求解每轮 $O(N(n^3+\dots))\approx10^6$ 标量运算，
...
| MINCO 子模块 | `src/core/MINCO/` | 与 `NMPC` 子模块并列的第二条后处理算法路径（已落地并接入 `PostProcessor::optimizeMinco`）：`MincoTrajectory`（多项式轨迹与 $K(T)$ 求解）、`BlockTridiagonalSolver`（块 Thomas 求解器）、`BicycleKinematicsExtractor`（阿克曼状态解析）、`MincoManeuverSegmenter`（前端解析与分段）、`MincoEsdfPenalty`（双重安全惩罚）、`MincoPreprocessor`（预处理粗优化）、`MincoSolver`（PHR-ALM 主求解器）、`SampleMincoTrajectory`（θ-s 轨迹离散化工具）、`MincoManeuverMelter`（机动融化与拓扑修剪），详见 3.7 节与 [docs/MINCO.md](MINCO.md) |
| iLQR 子模块 | `src/core/iLQR/` | 与 NMPC/MINCO 并列的第三条后处理算法路径（Milestone 001~009 已落地并接入 `PostProcessor::optimizeiLQR`，端到端效果攻坚 010/011 见 [docs/milestones.md](milestones.md)）：`iLQRReferenceBuilder`（预处理/参考构建）、`BicycleDynamics`（七维自行车动力学与解析雅可比）、`BoxQpSolver`（投影牛顿）、`iLQRCostEvaluator`/`iLQREsdfConstraint`（代价/约束求值层）、`MsIlqrSolver`（内层）、`AlOuterLoop`/`ApaILQRSolver`（AL 外层与求解编排）、`iLQRPostStage`（后处理与门控精化），详见 3.8 节与 [docs/iLQR.md](iLQR.md) |
| 场景组装 | `src/scene/` | `PlanningScene` 基类（持有一次后处理任务的完整上下文：环境+车辆+路径+配置）与三个并列场景实现：`NMPCPlanningScene`（NMPC 链路）、`MINCOPlanningScene`（MINCO 链路，效仿前者结构）、`ILQRPlanningScene`（iLQR 链路，同结构）；基类静态工厂 `LoadFromFile` 按场景配置的 `config_details_path` 所指详情 JSON 的 `"algorithm"` 字段运行时路由场景。配置约定：**每个算法一个配置详情 JSON**（`data/minco_config.json`/`data/nmpc_config.json`/`data/ilqr_config.json`，含 `"algorithm"` 路由字段与算法字段覆盖项），场景级 `data/config.json` 只承载 `data_file_path` 与 `config_details_path`，切换算法只改后者；算法无关的基类字段覆盖项由 `src/util/config_loader.h` 统一解析（当前映射 `outer_row_num`） |
...
flowchart TB
    ...
    spatial --> minco[core/MINCO]
    vehicle --> minco
    util --> minco
    spatial --> ilqr[core/iLQR 规划中]
    vehicle --> ilqr
    util --> ilqr
    minco --> scene[scene: PlanningScene/MINCOPlanningScene/NMPCPlanningScene]
    ilqr --> scene
    nmpc --> scene
    scene --> main[main.cpp]
    util --> main
...
`scene --> main` 边为编译期直接依赖：`main.cpp` 只包含 `MINCOPlanningScene`
（`minco_planning_scene.h`）与 `util` 头文件，经场景层接入 MINCO 路径；`minco --> scene`/
`nmpc --> scene` 为场景层对两条算法路径的并列依赖（`planning_scene.h` →
`post_processor.h` → MINCO/NMPC 头文件）。`main.cpp` 不再直接调用 NMPC 链路，
`nmpc --> main`/`preprocess --> main`/`minco --> main` 边已随 `main.cpp` 去除 NMPC 依赖的改造移除；
NMPC 路径继续由 `NMPCPlanningScene`、单元测试与 benchmark 独立调用。
...
| 2026-07-19 | 新增第二条后处理算法路径规划：3.7 节 MINCO/MINCO/PHR-ALM 优化领域模型（与 3.4 节 NMPC 并列，彻底改造自 arXiv:2409.07924）；第 4 节模块划分表新增 `src/core/MINCO/` 行；具体 Milestone 拆分见 [docs/milestones.md](milestones.md)，接口规划见 [docs/interfaces.md](interfaces.md) |
| 2026-07-21 | MINCO 路径全部落地并接入 `PostProcessor`：3.7 节标题移除"规划中"标记、收尾段落更新为真实链路（`PostProcessor::optimizeMinco` 编排五大组件，与 NMPC 路径并列互不干扰，`main.cpp` 生产入口行为不变）；第 4 节模块划分表 `src/core/MINCO/` 行更新为已实现组件清单；模块依赖图新增 `core/MINCO` 节点与 `minco --> main` 传递依赖边；四数据集验收对比见 [docs/MINCO.md](MINCO.md) 第三章 |
| 2026-07-21 | `main.cpp` 生产入口接入 MINCO 路径：新增 `MINCOPlanningScene`（效仿 `NMPCPlanningScene`），main 同时执行两条路径并生成三条轨迹对比图；第 4 节模块划分表"场景组装"行更新为 `PlanningScene` 基类 + 两个并列场景实现（顺带修正该行"空文件尚未实现"的过期描述）；3.7 节收尾段落同步更新 |
| 2026-07-21 | `main.cpp` 对比图内容调整：只绘制预处理轨迹与 MINCO 优化轨迹两条，NMPC 优化结果不再纳入横向对比（NMPC 场景仅为产出预处理轨迹保留）；3.7 节收尾段落同步 |
| 2026-07-22 | 一次代码复核发现 `MincoManeuverMelter` 的 `PIVOT` 处理与自行车模型自相矛盾，立项"收尾与独立化"系列改造（PIVOT 修复→`main.cpp` 改为对比 MINCO 自身预处理/优化后轨迹并去除对 NMPC 的依赖→`Trajectory::validate()` 新增运动学可行性校验→四数据集调参）；3.7 节收尾段落新增规划说明，接口层面的计划变更登记见 [docs/interfaces.md](interfaces.md)，PIVOT 问题分析见 [docs/known-limitations.md](known-limitations.md) 与 [docs/MINCO.md](MINCO.md) 3.4 节 |
| 2026-07-22 | `main.cpp` 移除对 `NMPCPlanningScene` 的依赖：离散化能力从 `MincoManeuverMelter` 拆出为独立纯函数工具 `SampleMincoTrajectory`（新增 `src/core/MINCO/minco_trajectory_sampler.h/.cpp`），`PostProcessorResult` 新增 `minco_preprocessed_traj` 字段、`MINCOPlanningScene` 新增 `mincoPreprocessedTraj()` 访问器；对比图改为 MINCO 自身"预处理粗优化轨迹（优化前） vs 主优化+机动融化后轨迹（优化后）"；3.7 节收尾段落与规划进展、3.6 节依赖说明、第 4 节模块划分表（MINCO 行新增离散化工具）与模块依赖图（新增 `scene` 节点，移除 `nmpc --> main`/`preprocess --> main`/`minco --> main` 边）同步更新 |
...
| 2026-07-22 | 四数据集调参完成：MINCO 采样轨迹补齐时间戳（运动学门对 MINCO 产出生效的前置）、运动学门新增低速跳过适配换挡尖点伪影、`MincoConfig::weight_jerk_s`（1.0→5.0）与 `weight_gear_cusp`（1000.0→50.0）默认值标定，四数据集全部满足三门合法性且段数/长度全面优于既有基线；新增 `tool/tune_minco.cpp` 调参驱动工具与 `apa_tune_minco` 构建目标，[docs/MINCO.md](MINCO.md) 第四章验收总表同步更新（新增"运动学可行"列） |
| 2026-07-28 | 新增第三条后处理算法路径规划：3.8 节 iLQR（MS-iLQR + AL）优化领域模型（与 3.4 节 NMPC、3.7 节 MINCO 并列，理论与工程约定见 [docs/iLQR.md](iLQR.md)）；第 4 节模块划分表新增 `src/core/iLQR/` 行、模块依赖图新增 `core/iLQR` 节点；Milestone 拆分（001~009）见 [docs/milestones.md](milestones.md)，接口规划见 [docs/interfaces.md](interfaces.md) |
| 2026-07-30 | iLQR 路径启动「效果攻坚」系列（Milestone 010/011）：3.8 节标题从「端到端调参进行中」改为「效果攻坚进行中」，新增「当前效果状态与攻坚方向」段落（M009 四数据集 2 收敛 + 2 回退，关键判断是「回退不等于数值优化失败」——`data1` 阶段一干净收敛却因阶段二未收敛被整体丢弃），三阶段流程图新增分级降级出口分支；第 4 节模块划分表 `src/core/iLQR/` 行同步。010 审计并重构后处理与通过性校验口径（修剪判据 Δδ/Δθ 量纲错配、剔除阈值与网格量纲不匹配、MINCO 3 项 vs iLQR 12 项的校验不对称、`maneuver_count` 逻辑倒挂、缺分级降级）；011 在正确口径上做四层方案与参数优化。接口层面的计划变更登记见 [docs/interfaces.md](interfaces.md) |

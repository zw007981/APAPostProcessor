...
AlOuterLoop::AlOuterLoop(const iLQRConfig& config) : config_(config) {
    // 调度参数校验：错误值会静默污染外层行为，构造期显式拒绝
    if (config_.outer_max_outer_iterations <= 0) {
        throw std::invalid_argument("AlOuterLoop: 外层迭代上限必须为正");
    }
    if (!(config_.outer_terminal_position_tol > 0.0) ||
        !(config_.outer_terminal_heading_tol_deg > 0.0) ||
        !(config_.outer_inequality_tol > 0.0) || !(config_.outer_defect_tol > 0.0)) {
        throw std::invalid_argument("AlOuterLoop: 终止判据容差必须为正");
    }
    if (!(config_.outer_mu_min > 0.0) || !std::isfinite(config_.outer_mu_max) ||
        !(config_.outer_mu_max >= config_.outer_mu_min)) {
        throw std::invalid_argument(
            "AlOuterLoop: 罚权重 clip 区间必须满足 0<μ_min<=μ_max");
    }
    if (!(config_.outer_first_round_mu > 0.0) || !(config_.outer_epsilon_mu > 0.0) ||
        !(config_.outer_amplitude_mu_initial > 0.0) ||
        !std::isfinite(config_.outer_amplitude_mu_initial)) {
        throw std::invalid_argument(
            "AlOuterLoop: 首轮罚权重/幅值初始罚权重与 ε_μ 必须为正有限");
    }
    if (!(config_.outer_mu_gate_kappa > 0.0 && config_.outer_mu_gate_kappa < 1.0) ||
        !(config_.outer_mu_growth_factor > 1.0) ||
        !(config_.outer_anneal_gamma > 0.0 && config_.outer_anneal_gamma < 1.0)) {
        throw std::invalid_argument(
            "AlOuterLoop: 门控/增长/退火参数必须满足 0<κ<1、φ>1、0<γ<1");
    }
    // ESDF 逐轮量级调度：增长率与上限都不得小于 1（小于 1 等于随轮次
    // 削弱避障，与本机制的意图相反），且必须为有限值
    if (!(config_.outer_esdf_scale_growth >= 1.0) ||
        !(config_.outer_esdf_scale_max >= 1.0) ||
        !std::isfinite(config_.outer_esdf_scale_growth) ||
        !std::isfinite(config_.outer_esdf_scale_max)) {
        throw std::invalid_argument(
            "AlOuterLoop: ESDF 逐轮量级增长率/上限必须为 >=1 的有限值");
    }
    // 幅值边界进入 g 公式（量测与代价求值层同口径），w_ref,0 进入退火基准
    if (!(config_.cost_v_max > 0.0) || !(config_.cost_a_max > 0.0) ||
        !(config_.cost_omega_max > 0.0) || !(config_.cost_delta_max > 0.0) ||
        !(config_.cost_weight_ref_base >= 0.0) ||
...
void AlOuterLoop::reset() {
    round_ = 0;
    mu_terminal_ = config_.outer_first_round_mu;
    mu_amplitude_ = config_.outer_amplitude_mu_initial;
    // 逐元素模式的状态在下一次更新/初始化乘子时按 5N 尺寸重建
    amplitude_mu_vec_.resize(0);
    prev_element_violation_.resize(0);
    mu_calibrated_ = config_.outer_first_round_mu;
...
double AlOuterLoop::trackingWeight() const {
    ...
    // 不衰减由代价层的退火豁免掩码保证）
    return config_.cost_weight_ref_base *
           std::pow(config_.outer_anneal_gamma, static_cast<double>(round_));
}
...
double AlOuterLoop::esdfScale() const {
    ...
    return std::min(
        config_.outer_esdf_scale_max,
        std::pow(config_.outer_esdf_scale_growth, static_cast<double>(round_)));
}
...
double AlOuterLoop::muAmplitude() const {
    ...
    if (config_.outer_amplitude_mu_per_element && amplitude_mu_vec_.size() > 0) {
...
AlConstraintSnapshot AlOuterLoop::measure(
    ...
    for (std::size_t k = 0; k < num_steps; ++k) {
        ...
        snapshot.amplitude_g(base + ILQR_AMP_V) =
            v * v - config_.cost_v_max * config_.cost_v_max;
        snapshot.amplitude_g(base + ILQR_AMP_A) =
            a * a - config_.cost_a_max * config_.cost_a_max;
        snapshot.amplitude_g(base + ILQR_AMP_OMEGA) =
            omega * omega - config_.cost_omega_max * config_.cost_omega_max;
        snapshot.amplitude_g(base + ILQR_AMP_DELTA_POS) =
            delta - config_.cost_delta_max;
    ...
    const double terminal_normalized_sq =
        std::pow(snapshot.terminal_c(0) / config_.outer_terminal_position_tol, 2.0) +
        std::pow(snapshot.terminal_c(1) / config_.outer_terminal_position_tol, 2.0) +
        std::pow(snapshot.terminal_c(2) /
                     (config_.outer_terminal_heading_tol_deg * PI / 180.0),
                 2.0) +
        std::pow(snapshot.terminal_c(3) / config_.cost_v_max, 2.0) +
        std::pow(snapshot.terminal_c(4) / config_.cost_a_max, 2.0);
...
AlTerminationCheck AlOuterLoop::checkTermination(
    ...
    check.terminal_ok =
        snapshot.terminal_position_error <= config_.outer_terminal_position_tol &&
        snapshot.terminal_heading_error_deg <= config_.outer_terminal_heading_tol_deg;
    check.inequality_ok =
        snapshot.max_amplitude_violation <= config_.outer_inequality_tol;
    check.defect_ok = snapshot.defect_norm_inf <= config_.outer_defect_tol;
    return check;
}
...
bool AlOuterLoop::update(const AlConstraintSnapshot& snapshot, double base_cost,
    ...
    if (!mu_calibrated_flag_) {
        mu_terminal_ =
            Clip(base_cost / std::max(snapshot.terminal_c.squaredNorm(),
                                      config_.outer_epsilon_mu),
                 config_.outer_mu_min, config_.outer_mu_max);
    ...
    const bool increase_terminal =
        ...
        terminal_violation > config_.outer_mu_gate_kappa * prev_terminal_violation_;
    const bool increase_amplitude =
        prev_amplitude_violation_ >= 0.0 &&
        amplitude_violation > config_.outer_mu_gate_kappa * prev_amplitude_violation_;
    if (increase_terminal) {
        mu_terminal_ =
            std::min(config_.outer_mu_growth_factor * mu_terminal_, config_.outer_mu_max);
    }
    bool amplitude_grew = false;
    if (config_.outer_amplitude_mu_per_element) {
        // 逐元素门控：仅当本元素的归一化违反度未充分下降才提升其 μ_j
        const auto n = snapshot.amplitude_g.size();
        if (amplitude_mu_vec_.size() != n) {
            amplitude_mu_vec_.setConstant(n, config_.outer_amplitude_mu_initial);
        ...
        for (Eigen::Index i = 0; i < n; ++i) {
            ...
            if (prev_element_violation_(i) >= 0.0 &&
                active > config_.outer_mu_gate_kappa * prev_element_violation_(i)) {
                amplitude_mu_vec_(i) =
                    std::min(config_.outer_mu_growth_factor * amplitude_mu_vec_(i),
                             config_.outer_mu_max);
                amplitude_grew = true;
            }
        ...
        if (increase_amplitude) {
            mu_amplitude_ = std::min(config_.outer_mu_growth_factor * mu_amplitude_,
                                     config_.outer_mu_max);
            amplitude_grew = true;
        }
        multipliers->amplitude_mu.setConstant(mu_amplitude_);

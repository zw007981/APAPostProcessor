...
// 阶段一求解报告
struct ApaILQRReport {
    // 求解状态
    ApaILQRStatus status{ApaILQRStatus::MAX_OUTER_ITERATIONS};
    // 逐轮诊断历史
    std::vector<ApaILQROuterRecord> history;
    // 外层轮数
...
struct ApaILQRStageOneResult {
    iLQRAlignedVec<iLQRState> states;
    iLQRAlignedVec<iLQRControl> controls;
    // 求解报告
    ApaILQRReport report;
    // 供阶段二对偶热启动
    iLQRCostMultiplierState final_multipliers;
};
struct ApaILQRStageTwoResult {
    iLQRAlignedVec<iLQRState> states;
    iLQRAlignedVec<iLQRControl> controls;
    // 求解报告
    ApaILQRReport report;
...
// APA-iLQR 求解编排入口：阶段一全局软化 + 阶段二门控精化，δ 投影防 tanδ
// 奇异区，内层溢出冷重启一次再判死
class ApaILQRSolver {
   public:
    ...
    // 为阶段一终态乘子（对偶热启动）
    ApaILQRStageTwoResult solveStageTwo(
   ...
   protected:
    ...
    GatingSnapshot measureGating(const iLQRGatingPlan& plan,
    ...
    bool updateGating(const GatingSnapshot& snapshot,
                      iLQRCostMultiplierState* multipliers, double* gating_mu,
    ...
    void runOuterLoop(const iLQRReference& reference, AlOuterLoop* outer,
                      ...
                      iLQRAlignedVec<iLQRState>* warm_virtual_controls,

...
#include "MINCO_THETA_S/minco_theta_s_esdf_penalty.h"
#include "MINCO_THETA_S/minco_theta_s_maneuver_melter.h"
#include "MINCO_THETA_S/minco_theta_s_maneuver_segmenter.h"
#include "MINCO_THETA_S/minco_theta_s_preprocessor.h"
#include "MINCO_THETA_S/minco_theta_s_solver.h"
#include "MINCO_THETA_S/minco_theta_s_steer_padding.h"
#include "MINCO_THETA_S/minco_theta_s_trajectory_sampler.h"
...
namespace {
// 优化结果允许的最大碰撞深度 (m)：NMPC 与 MINCO_THETA_S 两条路径共用同一质量门
...
// 输出等距重采样：MINCO_THETA_S 轨迹按时间等分采样，点距随速度变化（低速蠕动区
...
PostProcessorResult PostProcessor::optimize(
    ...
    // 交付曲率统一口径填充（几何 kappa + kappa_kinematic），与 MINCO_THETA_S/iLQR
    // 交付轨迹语义一致
...
// MINCO_THETA_S 专有字段覆盖项：仅覆盖显式出现的字段，未出现的保持构造默认值
void LoadMincoThetaSConfigOverrides(const nlohmann::json& details,
                              MincoThetaSConfig* config) {
    if (config == nullptr) {
        throw std::invalid_argument(
            "LoadMincoThetaSConfigOverrides received null config!!!");
    }
    // ESDF 双重安全惩罚节（与 MincoThetaSConfig 扁平字段同名）
    const auto& esdf = JsonSectionOrEmpty(details, "esdf");
...
MincoThetaSConfig PostProcessor::DeriveKinematicsConfig(
    const VehicleParams& vehicle_params, const MincoThetaSConfig& minco_theta_s_config) {
    MincoThetaSConfig config = minco_theta_s_config;
    config.wheelbase = vehicle_params.wheelbase;
    // max_velocity 是 MINCO_THETA_S 路径独立配置量，保持配置原值不覆盖
...
PostProcessorResult PostProcessor::optimizeMincoThetaS(
    const Path& init_path, const MincoThetaSConfig& minco_theta_s_config) const {
    PostProcessorResult result;
    const auto t_start = std::chrono::steady_clock::now();
    ...
    const auto finish = [&result, t_start]() {
        result.total_time_ms = std::chrono::duration<double, std::milli>(
                                   ...
                                   .count();
        // 先填充输出标签再统计
        result.algorithm = "minco_theta_s";
        result.output_level = result.success ? OutputLevel::kFullSuccess
                                             : OutputLevel::kFallback;
    ...
    try {
        // 空路径无法提取起点锚点，直接显式失败
        if (init_path.empty()) {
            result.message = "MINCO_THETA_S input path is empty";
            return finish();
        }
        // 世界坐标还原积分的锚点：初始路径首点
        ...
        const MincoThetaSConfig derived =
            DeriveKinematicsConfig(vehicle_params_, minco_theta_s_config);
        const BicycleKinematicsExtractor kinematics(derived);
        // 1) 前端解析与分段：换挡打断 + 空间等距降采样
        const MincoThetaSManeuverSegmenter segmenter(derived);
        const auto estimates = segmenter.segment(init_path);
        // 2) 预处理粗优化：把初值拉近前端路径并满足运动学约束
        const MincoThetaSPreprocessor preprocessor(derived);
        const auto pre_result =
            preprocessor.preprocess(estimates, start_position);
        if (!pre_result.success) {
            result.message = "MINCO_THETA_S preprocessing failed (max endpoint error " +
        ...
        if (solve_result.trajectory.numSegments() == 0) {
            ...
            result.message = "MINCO_THETA_S solve failed: no valid trajectory produced";
            return finish();
        }
        // 4) 机动融化与拓扑修剪：剔除废段、同向合并，产出采样 Path
        const MincoThetaSManeuverMelter melter(derived);
        ...
        if (optimized.empty()) {
            result.message = "MINCO_THETA_S melt produced empty path";
            return finish();
        }
        ...
        MincoThetaSConfig padding_config = derived;
        ...
        if (padding_stats.windows_legalized > 0 ||
            ...
            LOG_FMT_INFO("MINCO_THETA_S steer padding: legalized={}, skipped={}",
        ...
        if (!IsPathFinite(optimized)) {
            result.message = "MINCO_THETA_S output rejected: non-finite samples";
            return finish();
        }
        ...
        if (max_collision > kMaxAcceptableCollisionDepth) {
            LOG_FMT_WARN(
                "MINCO_THETA_S collision depth {:.4f}m exceeds threshold, rejecting",
                max_collision);
            result.message = "MINCO_THETA_S collision depth " +
                             std::to_string(max_collision) +
                             " m exceeds threshold, rejecting";
            return finish();
        }
        // 终点精度以采样后路径末点相对初始路径末点（即停车目标位姿）度量；
        // 阈值复用主求解器的双指标收敛判据，与 MINCO_THETA_S 收敛定义保持一致
        ...
        optimized.finalize();
        // 填充 MINCO_THETA_S 轨迹视图（与输出 Path 同序采样点，
        // 携带 v/a/delta/delta_dot，未携带时间戳），供可视化与下游消费
        ...
        result.success = true;
        result.message =
            solve_result.status == MincoThetaSSolverStatus::CONVERGED
                ? "MINCO_THETA_S converged"
                : "MINCO_THETA_S did not fully converge after " +
        ...
        LOG_FMT_WARN("optimizeMincoThetaS exception: {}", e.what());
        result.message = std::string("MINCO_THETA_S attempt failed: ") + e.what();
    }
    return finish();
}

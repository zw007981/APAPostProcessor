...
DdpEsdfConstraint::DdpEsdfConstraint(
...
DdpEsdfPoseCost DdpEsdfConstraint::evaluate(double x, double y,
                                            double theta) const {
    DdpEsdfPoseCost result;
    ...
    result.gradient(DDP_IDX_X) = grad3.x();
    result.gradient(DDP_IDX_Y) = grad3.y();
    result.gradient(DDP_IDX_THETA) = grad3.z();
    result.hessian.topLeftCorner<3, 3>() = hess3;
    return result;
}

DdpEsdfCircleConstraint DdpEsdfConstraint::evaluateCircle(

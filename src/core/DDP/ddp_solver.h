...
// DDP 单次求解的完整产出，由 PostProcessor 映射为统一 PostProcessorResult
struct DdpSolverResult {
    // 最终优化轨迹
...
// DDP 完整链路编排器：参考构建 → 阶段一 → 后处理与阶段二。无状态，每次
// optimizeSinglePass() 按需构造组件
class DdpSolver {
   public:
    // 构造时持有车辆参数、足迹模型与 ESDF 地图的只读引用
    DdpSolver(const VehicleParams& vehicle_params,
              const VehicleFootprintModel& footprint_model,
              const ESDFMap& esdf_map);

    // 单遍完整链路，不含双候选择优与 RS 短接回退（那两层由 PostProcessor 调度）
    DdpSolverResult optimizeSinglePass(const Path& init_path,
                                       const DdpConfig& config) const;
   ...
   protected:
    ...
    // 计算开销，而足迹模型在 DdpSolver 生命周期内不变。缓存避免双候选
    // 场景重复提取
    mutable std::unique_ptr<DdpEsdfConstraint> esdf_cache_;
    // 上次缓存对应的 ESDF 配置，用于判断是否需要重建
    mutable DdpEsdfConstraintConfig last_esdf_config_;
};

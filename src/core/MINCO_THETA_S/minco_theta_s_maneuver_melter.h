...
enum class MincoThetaSMeltClass {
    // 正常机动段（|Δs| 不低于融化弧长阈值）
    NORMAL,
...
struct MincoThetaSManeuverMeltInfo {
    // 分类结果
    MincoThetaSMeltClass classification = MincoThetaSMeltClass::NORMAL;
...
struct MincoThetaSMeltResult {
    ...
    std::vector<MincoThetaSManeuverMeltInfo> maneuver_infos;
};
// 机动融化与拓扑修剪：MINCO_THETA_S 收敛后的拓扑级后处理。融化判据直接取 θ-s
...
// 首尾段保护：首段决定车辆当前位姿、末段承载 MINCO_THETA_S 精确收敛的终点
...
class MincoThetaSManeuverMelter {
   public:
    // 构造并校验配置：阈值必须为正有限值、采样数 >= 2，非法抛
    // std::invalid_argument
    explicit MincoThetaSManeuverMelter(const MincoThetaSConfig& config);
    // 主入口：融化检测 → 方向标记 → 复用 topology_cleaner 第二遍重建。
    // 输入非法（空估计/空 Maneuver/估计与轨迹段数不一致/非有限起点）抛
    // std::invalid_argument
    MincoThetaSMeltResult meltAndPrune(
    ...
    std::vector<MincoThetaSManeuverMeltInfo> detectMelting(
        const MincoThetaSTrajectory& trajectory,

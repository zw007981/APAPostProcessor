...
// MINCO_THETA_S 配置
struct MincoThetaSConfig : public Config {
    ...
    // ===== ESDF 双重安全惩罚（MincoThetaSEsdfPenalty）=====
    // 硬安全边界 margin_safe (m)
    ...
    // ===== 前端解析与降采样（MincoThetaSManeuverSegmenter）=====
    ...
    // ===== 机动融化与拓扑修剪（MincoThetaSManeuverMelter）=====
    ...
    // ===== 预处理粗优化（MincoThetaSPreprocessor）=====
    // 逐段终点对 p_w0 的二次跟踪权重
    ...
    // ===== PHR-ALM 主流程（MincoThetaSSolver，solver_ 前缀）=====
    // 终点位置收敛阈值 e_pos (m)
    ...
    // ===== 停驻窗口合法化改写（MincoThetaSSteerPadding，pad_ 前缀）=====

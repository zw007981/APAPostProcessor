#include "minco_theta_s_maneuver_melter.h"

#include <cmath>
#include <stdexcept>

#include "../../util/topology_cleaner.h"
#include "minco_theta_s_trajectory_sampler.h"

namespace apa_post_processor {
MincoThetaSManeuverMelter::MincoThetaSManeuverMelter(const MincoThetaSConfig& config)
    : config_(config) {
    if (!(config_.melt_arc_threshold > 0.0) ||
        ...
        throw std::invalid_argument(
            "MincoThetaSConfig 融化阈值必须为正有限值");
    }
    if (config_.samples_per_segment < 2) {
        throw std::invalid_argument("MincoThetaSConfig 采样数须 >= 2");
    }
}

MincoThetaSMeltResult MincoThetaSManeuverMelter::meltAndPrune(
    ...
    CheckMincoThetaSSampleStructure(trajectory, estimates);
    if (!start_position.allFinite()) {
        throw std::invalid_argument("起点世界坐标必须为有限值");
    }
    MincoThetaSMeltResult result;
    ...
    for (std::size_t m = 0; m < maneuvers.size(); ++m) {
        switch (result.maneuver_infos[m].classification) {
            ...
            case MincoThetaSMeltClass::PIVOT:
                ...
                ++result.pivot_count;
                break;
            case MincoThetaSMeltClass::NORMAL:
                break;
        }
    }
...
std::vector<MincoThetaSManeuverMeltInfo> MincoThetaSManeuverMelter::detectMelting(
    const MincoThetaSTrajectory& trajectory,
    const std::vector<MincoThetaSManeuverEstimate>& estimates) const {
    CheckMincoThetaSSampleStructure(trajectory, estimates);
    std::vector<MincoThetaSManeuverMeltInfo> infos;
    ...
    for (std::size_t m = 0; m < estimates.size(); ++m) {
        ...
        const double theta_end =
            trajectory
                .evaluateSegment(last_segment,
                                 trajectory.duration(last_segment), 0)
                .x();
        MincoThetaSManeuverMeltInfo info;
        ...
        // 被重分类为 PIVOT——首段承载车辆当前位姿锚点、末段承载 MINCO_THETA_S 精确收敛
        ...
        if (!endpoint && tiny_arc &&
            ...
            info.classification = MincoThetaSMeltClass::PIVOT;
        } else if (tiny_arc && !endpoint) {
            info.classification = MincoThetaSMeltClass::MELTED;
        }

...
struct MincoThetaSSegmentEstimate {
...
struct MincoThetaSManeuverEstimate {
    ...
    // 微观段序列（至少 1 段）
    std::vector<MincoThetaSSegmentEstimate> segments;
};
// 前端 Hybrid A* 路径解析器：把粗糙 Path 解析为 MINCO_THETA_S 优化所需的初始 M 段
...
class MincoThetaSManeuverSegmenter {
   public:
    // 构造并校验配置：四个数值字段均必须为正有限值，非法抛
    // std::invalid_argument
    explicit MincoThetaSManeuverSegmenter(const MincoThetaSConfig& config);
    // 把输入 Path 解析为每个 Maneuver 的初始段估计；Path 为空抛
    // std::invalid_argument
    std::vector<MincoThetaSManeuverEstimate> segment(const Path& path) const;
    // 当前配置（只读）
   ...
   protected:
    ...
    MincoThetaSManeuverEstimate segmentManeuver(const Maneuver& maneuver,
    ...
    std::vector<MincoThetaSManeuverEstimate> FuseShortManeuvers(

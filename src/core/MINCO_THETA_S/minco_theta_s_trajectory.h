...
struct MincoThetaSBoundaryCondition {
    double pos{0.0};
    double vel{0.0};
    double acc{0.0};
};
// θ-s 运动状态空间的二维边界条件（theta 与 s 各一组）
struct MincoThetaSBoundaryCondition2d {
    MincoThetaSBoundaryCondition theta;
    MincoThetaSBoundaryCondition s;
};
// 单点 0~2 阶批量求值结果（实时间导数；x 分量为 θ 维、y 分量为 s 维）。
// 供物理约束装配等热路径一次取全三阶导数，替代三次单阶 evaluateSegment
// 调用（三行基函数共享幂次链，逐位等价有单测钉住）
struct MincoThetaSSegmentSample {
    // 0 阶值（位置量）：(θ, s)
...
struct MincoThetaSSegmentEndOrders34 {
...
class MincoThetaSTrajectory {
   public:
    ...
    // std::runtime_error。
    void setTrajectory(const MincoThetaSBoundaryCondition2d& start,
                       const MincoThetaSBoundaryCondition2d& end,
    ...
    // std::logic_error
    MincoThetaSSegmentSample evaluateSegmentOrders02(int segment_index,
    ...
    MincoThetaSSegmentEndOrders34 evaluateSegmentEndOrders34(

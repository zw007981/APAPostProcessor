...
// 预处理问题装配数据：由 MincoThetaSManeuverSegmenter 的初值估计一次性装配，供
// L-BFGS 代价函数反复求值时复用，避免每次求值重新展开。
struct MincoThetaSPreprocessorProblem {
    // 起点边界条件（车辆起步为零速零加速度）
...
// 预处理粗优化结果
struct MincoThetaSPreprocessorResult {
...
// 预处理粗优化器：两阶段优化流程的第一阶段。以 MincoThetaSManeuverSegmenter 的
// 初值估计为输入，在松收敛阈值下求解 J_pre（不含 ESDF 碰撞惩罚与外层
// MINCO_THETA_S 乘子逻辑），把初值拉近前端路径并满足运动学约束，输出供主优化
// 使用的 MINCO_THETA_S 初值。决策变量布局：[2(M-1) 个内部航点 (θ,s) 分量,
// M 个时间重参数化变量 τ_i, 终点弧长 s_f]，梯度经 K(T)^{-T} 伴随反传
// （内部航点/终点弧长）与 K(T) 行对 T 的解析依赖（时间变量）。热路径经
// 预分配工作区复用堆容量（mutable 成员），同一实例的求解/求值不可并发
// 调用。
class MincoThetaSPreprocessor {
   public:
    // 构造并校验配置：非法配置抛 std::invalid_argument（运动学参数的合法性
    // 由 BicycleKinematicsExtractor 构造时校验）
    explicit MincoThetaSPreprocessor(const MincoThetaSConfig& config);
    ...
    // 不静默返回垃圾结果
    MincoThetaSPreprocessorResult preprocess(
   ...
   protected:
    ...
    // 求导"的节点一致性。物理约束采样并入辛普森偶数节点（配置校验保证
    // (physics_samples-1) 整除 simpson_subintervals），节点批量采样结果
    // 同时供物理惩罚与后缀反传消费，不重复求值
    ...
    // 装配问题数据并逐项校验输入合法性
    MincoThetaSPreprocessorProblem buildProblem(
        const std::vector<MincoThetaSManeuverEstimate>& estimates,
        const Eigen::Vector2d& start_position) const;
    // 求值 J_pre 代价与解析梯度（L-BFGS 函数对象；gradient 必须已分配且
    // 维数等于 problem.variableCount()）
    double evaluateCostAndGradient(const MincoThetaSPreprocessorProblem& problem,
                                   const Eigen::VectorXd& x,
                                   Eigen::VectorXd* gradient) const;
    // 由决策变量重建 MINCO_THETA_S 轨迹（维数不匹配抛 std::invalid_argument）
    MincoThetaSTrajectory buildTrajectory(const MincoThetaSPreprocessorProblem& problem,

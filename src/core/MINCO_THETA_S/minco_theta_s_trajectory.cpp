#include "minco_theta_s_trajectory.h"
...
void MincoThetaSTrajectory::setTrajectory(
    const MincoThetaSBoundaryCondition2d& start, const MincoThetaSBoundaryCondition2d& end,
    const std::vector<Eigen::Vector2d>& waypoints,
    const std::vector<double>& durations) {
    const int num_segments = static_cast<int>(durations.size());
    if (num_segments < 1) {
        throw std::invalid_argument("MincoThetaSTrajectory 至少需要一个多项式段");
    }
...
Eigen::Vector2d MincoThetaSTrajectory::evaluate(double t, int order) const {
...
Eigen::Vector2d MincoThetaSTrajectory::evaluateSegment(int segment_index,
                                                 double local_time,
                                                 int order) const {
    checkEvaluable(order);
...
MincoThetaSSegmentSample MincoThetaSTrajectory::evaluateSegmentOrders02(
    ...
    DerivativeBasisRows012(clamped_time / segment_duration, segment_duration,
                           &row0, &row1, &row2);
    MincoThetaSSegmentSample sample;
...
MincoThetaSSegmentEndOrders34 MincoThetaSTrajectory::evaluateSegmentEndOrders34(
    ...
    MincoThetaSSegmentEndOrders34 result;
...
double MincoThetaSTrajectory::duration(int segment_index) const {
    return durations_.at(segment_index);
}

double MincoThetaSTrajectory::totalDuration() const {
    return cumulative_durations_.empty() ? 0.0 : cumulative_durations_.back();
}

double MincoThetaSTrajectory::finalArcLengthAdjointGradient(
    const CoeffMatrix& dJ_over_dcs) const {
    if (!solver_.isFactorized()) {
        throw std::logic_error("MincoThetaSTrajectory 尚未构建轨迹");
    }
...
MincoThetaSTrajectory::CoeffMatrix MincoThetaSTrajectory::solveAdjoint(
...
double MincoThetaSTrajectory::TauToDuration(double tau) {
...
double MincoThetaSTrajectory::DurationToTau(double duration) {
    if (!std::isfinite(duration) || duration <= 0.0) {
...
double MincoThetaSTrajectory::TauToDurationDerivative(double tau) {
    if (tau > 0.0) {
        return tau + 1.0;
    }
    const double shifted = 1.0 - tau;
...
double MincoThetaSTrajectory::DurationToTauDerivative(double duration) {
    if (!std::isfinite(duration) || duration <= 0.0) {
        throw std::invalid_argument("段时长必须为正有限值");
    }
    if (duration > 1.0) {
...
void MincoThetaSTrajectory::AssembleK(
    const std::vector<double>& durations,
...
MincoThetaSTrajectory::CoeffMatrix MincoThetaSTrajectory::AssembleRhs(
...
Eigen::Matrix<double, 1, MincoThetaSTrajectory::COEFFS_PER_SEG>
MincoThetaSTrajectory::DerivativeBasisRow(double tau_norm, int order,
                                    double duration) {
...
void MincoThetaSTrajectory::DerivativeBasisRows012(
...
int MincoThetaSTrajectory::locateSegment(double t) const {
...
void MincoThetaSTrajectory::checkEvaluable(int order) const {
    if (!solver_.isFactorized()) {
        throw std::logic_error("MincoThetaSTrajectory 尚未构建轨迹");
    }

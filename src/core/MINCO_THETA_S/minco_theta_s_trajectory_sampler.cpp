...
std::vector<Maneuver> SampleMincoThetaSTrajectory(
    ...
    auto append_sample = [&](std::vector<TrajectoryPoint>* points,
                             ...
                             double global_time) {
        ThetaSSample sample;
        // 0~2 阶批量求值一次取全（三行基函数共享幂次链），替代三次单阶调用
        const MincoThetaSSegmentSample evals =
            trajectory.evaluateSegmentOrders02(global_segment, local_time);
        sample.theta = evals.d0.x();
        sample.theta_dot = evals.d1.x();
        sample.theta_ddot = evals.d2.x();
        sample.s = evals.d0.y();
        sample.s_dot = evals.d1.y();
        sample.s_ddot = evals.d2.y();
...
void CheckMincoThetaSSampleStructure(
    const MincoThetaSTrajectory& trajectory,
    const std::vector<MincoThetaSManeuverEstimate>& estimates) {
    if (estimates.empty()) {

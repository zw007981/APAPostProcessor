...
MincoThetaSManeuverSegmenter::MincoThetaSManeuverSegmenter(const MincoThetaSConfig& config)
    : config_(config) {
...
std::vector<MincoThetaSManeuverEstimate> MincoThetaSManeuverSegmenter::segment(
...
std::vector<MincoThetaSManeuverEstimate> MincoThetaSManeuverSegmenter::FuseShortManeuvers(
    std::vector<MincoThetaSManeuverEstimate> estimates) const {
    ...
    std::vector<MincoThetaSManeuverEstimate> result;
    result.reserve(n);
    ...
    for (std::size_t i = 0; i < n; ++i) {
        ...
        MincoThetaSManeuverEstimate kept = std::move(estimates[i]);
...
MincoThetaSManeuverEstimate MincoThetaSManeuverSegmenter::segmentManeuver(
    ...
    *prev_theta = estimate.start_theta;
    ...
    if (num_points == 1) {
        ...
        MincoThetaSSegmentEstimate segment;
...
double MincoThetaSManeuverSegmenter::DirectionSign(Direction direction) {
    switch (direction) {
...
double MincoThetaSManeuverSegmenter::UnwrapAngle(double theta, double reference) {

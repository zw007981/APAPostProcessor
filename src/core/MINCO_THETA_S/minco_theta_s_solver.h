...
#include "minco_theta_s_config.h"
#include "minco_theta_s_esdf_penalty.h"
#include "minco_theta_s_maneuver_segmenter.h"
#include "minco_theta_s_preprocessor.h"
#include "minco_theta_s_trajectory.h"

namespace apa_post_processor {
// PHR-ALM 主求解器外层迭代状态
enum class MincoThetaSSolverStatus {
    // 终点位置/朝向双指标同时满足收敛判据
    CONVERGED,
...
struct MincoThetaSSolverProblem {
    ...
    // 终点目标位姿（MINCO_THETA_S 终端项的 x_target/y_target/θ_target）
...
struct MincoThetaSMultiplierState {
    // 终点 x 向乘子 λ_x
    double lambda_x = 0.0;
    // 终点 y 向乘子 λ_y
    double lambda_y = 0.0;
...
struct MincoThetaSCostBreakdown {
    // 总代价 J_ρ = J_s' + minco_theta_s_terminal
    double total = 0.0;
    // 基础平滑目标 J_0 与全部不等式软约束惩罚之和（不含 MINCO_THETA_S 终端项）
...
struct MincoThetaSOuterIterationRecord {
    // 外层迭代序号（0 基）
    int outer_index = 0;
...
// PHR-ALM 主优化结果
struct MincoThetaSSolverResult {
    // 整体成功：双指标收敛判据满足（status == CONVERGED）
    bool success = false;
    // 外层迭代终止状态（失败路径的显式反馈，不静默返回垃圾结果）
    MincoThetaSSolverStatus status = MincoThetaSSolverStatus::MAX_OUTER_ITERATIONS;
    ...
    // 逐轮外层迭代记录
    std::vector<MincoThetaSOuterIterationRecord> history;
};
...
// 不可并发调用。
class MincoThetaSSolver {
   public:
    // 构造并校验配置：非法配置抛 std::invalid_argument（运动学参数的合法性
    // 由 BicycleKinematicsExtractor 构造时校验，ESDF 惩罚配置的合法性由
    // MincoThetaSEsdfPenalty 构造时校验）
    explicit MincoThetaSSolver(const MincoThetaSConfig& config);
    ...
    MincoThetaSSolverResult solve(const std::vector<MincoThetaSManeuverEstimate>& estimates,
    ...
    // 当前配置（只读）
    const MincoThetaSConfig& config() const { return config_; }
   ...
   protected:
    ...
    struct SimpsonNodeData {
        ...
        std::vector<MincoThetaSSegmentSample> node_samples;
        // 每段的辛普森积分位移（世界系）
    ...
    // 装配问题数据并逐项校验输入合法性
    MincoThetaSSolverProblem buildProblem(
    ...
    double evaluateCostAndGradient(const MincoThetaSSolverProblem& problem,
    ...
    // 由决策变量重建 MINCO_THETA_S 轨迹（维数不匹配抛 std::invalid_argument）
    MincoThetaSTrajectory buildTrajectory(const MincoThetaSSolverProblem& problem,

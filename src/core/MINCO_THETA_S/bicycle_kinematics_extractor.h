...
// 单时刻 θ-s 轨迹采样点：位置与各阶导数（可由 MincoThetaSTrajectory::evaluate 逐阶
// 查询组装）
struct ThetaSSample {
    // 朝向角 (rad)
    double theta{0.0};
...
// MincoThetaSTrajectory 给出的 θ,θ̇,θ̈,s,ṡ,s̈，输出 v,a,δ,δ̇ 及四个物理约束的
...
class BicycleKinematicsExtractor {
   public:
    ...
    explicit BicycleKinematicsExtractor(const MincoThetaSConfig& config);

...
std::pair<double, double> DifferentialFlatnessSolver::computeSinglePoint(
    ...
    double y_d3, double v, double wheelbase, double denominator_epsilon) const {
    // 参数速度模方，是曲率/曲率导数公式的公共分母
    ...
    // d(delta)/dt = L * v / (1 + (kappa*L)^2) * dkappa/ds
    const double one_plus_kappa_l_sq =
        1.0 + (kappa * wheelbase) * (kappa * wheelbase);
    const double delta_dot = (wheelbase * v / one_plus_kappa_l_sq) * dkappa_ds;
...
DifferentialFlatnessResult DifferentialFlatnessSolver::computeFlatOutputs(
    ...
    result.success = true;
    result.status_msg = "OK: " + std::to_string(n_points) + " points computed";
    for (std::size_t i = 0; i < n_points; ++i) {
        const auto [delta, delta_dot] = computeSinglePoint(
            input.x_d1[i], input.x_d2[i], input.x_d3[i], input.y_d1[i],
            input.y_d2[i], input.y_d3[i], input.v[i], vehicle_params.wheelbase,
            config_.curvature_denominator_epsilon);
        TrajectoryPoint point(input.x[i], input.y[i], input.theta[i]);

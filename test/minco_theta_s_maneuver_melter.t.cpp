...
#include "core/MINCO_THETA_S/minco_theta_s_maneuver_melter.h"
#include "core/MINCO_THETA_S/bicycle_kinematics_extractor.h"
...
// 人造场景：MincoThetaSTrajectory（K(T) 边界精确满足，段连接处 θ/s 即构造值）与
...
FabricatedScene Fabricate(
    ...
    MincoThetaSBoundaryCondition2d start;
    ...
    int global_index = 0;
    for (const auto& segment_specs : maneuver_specs) {
        MincoThetaSManeuverEstimate estimate;
...
TEST(MincoThetaSManeuverMelterTest, MeltedSegmentIsPrunedAndMerged) {
    const FabricatedScene scene = Fabricate({
        {Direction::FORWARD, 0.0, 1.0, 2.0},
        {Direction::BACKWARD, 0.01, -0.01, 1.0},
        {Direction::FORWARD, 0.0, 1.0, 2.0},
    });
    const MincoThetaSManeuverMelter melter(MincoThetaSConfig{});
    const std::vector<MincoThetaSManeuverMeltInfo> infos =
        melter.detectMelting(scene.trajectory, scene.estimates);
    ASSERT_EQ(infos.size(), 3U);
    EXPECT_EQ(infos[0].classification, MincoThetaSMeltClass::NORMAL);
    EXPECT_EQ(infos[1].classification, MincoThetaSMeltClass::MELTED);
    EXPECT_EQ(infos[2].classification, MincoThetaSMeltClass::NORMAL);
    ...
    const MincoThetaSMeltResult result = melter.meltAndPrune(
...
// 首尾位姿必须逐位一致。
TEST(MincoThetaSManeuverMelterTest, TerminalPoseNotDisturbedByPruning) {
    const FabricatedScene scene = Fabricate({
        {Direction::FORWARD, 0.0, 1.0, 2.0},
        {Direction::BACKWARD, 0.01, -0.01, 1.0},
        {Direction::FORWARD, 0.0, 1.0, 2.0},
    ...
    const BicycleKinematicsExtractor kinematics = MakeKinematics();
    const MincoThetaSManeuverMelter melter(MincoThetaSConfig{});
    const MincoThetaSMeltResult pruned = melter.meltAndPrune(
        scene.trajectory, scene.estimates, {0.0, 0.0}, kinematics);
    ASSERT_TRUE(pruned.pruned);
    // 对照组：阈值压到任何段都无法触发（最小 |Δs|=0.01 > 1e-9），不修剪
    MincoThetaSConfig no_melt_config;
    no_melt_config.melt_arc_threshold = 1e-9;
    const MincoThetaSManeuverMelter no_melt_melter(no_melt_config);
    const MincoThetaSMeltResult unpruned = no_melt_melter.meltAndPrune(
        scene.trajectory, scene.estimates, {0.0, 0.0}, kinematics);
...
TEST(MincoThetaSManeuverMelterTest, OppositeDirectionNeighborsNeverMerged) {
    const FabricatedScene scene = Fabricate({
        {Direction::FORWARD, 0.0, 1.0, 2.0},
...
TEST(MincoThetaSManeuverMelterTest, PivotSegmentIsPreserved) {
    const FabricatedScene scene = Fabricate({
        {Direction::FORWARD, 0.0, 1.0, 2.0},
    ...
    const std::vector<MincoThetaSManeuverMeltInfo> infos =
        melter.detectMelting(scene.trajectory, scene.estimates);
    ASSERT_EQ(infos.size(), 3U);
    EXPECT_EQ(infos[1].classification, MincoThetaSMeltClass::PIVOT);
...
// （梯形平均），二者必须一致。
TEST(MincoThetaSManeuverMelterTest, PivotSegmentSatisfiesBicycleKinematics) {
    const FabricatedScene scene = Fabricate({
        {Direction::FORWARD, 0.0, 1.0, 2.0},
        {Direction::BACKWARD, 0.5, -0.01, 1.0},
        {Direction::FORWARD, 0.0, 1.0, 2.0},
    ...
    const MincoThetaSManeuverMelter melter(config);
    const MincoThetaSMeltResult result = melter.meltAndPrune(
        scene.trajectory, scene.estimates, {0.0, 0.0}, kinematics);
...
// 首段决定车辆当前位姿、末段承载 MINCO_THETA_S 精确收敛的终点等式约束（MINCO_THETA_S.md
// 2.6.3 节"微调必须保持已收敛的终点 x,y,θ 等式约束不被扰动"），二者
// 只允许参与同向合并，不允许被当作废段压平。
TEST(MincoThetaSManeuverMelterTest, FirstAndLastManeuverNeverRemoved) {
    const FabricatedScene scene = Fabricate({
...
// 约束（MINCO_THETA_S.md 2.6.3 节"微调必须保持已收敛的终点等式约束不被扰动"），
// 其真实运动方向是输出语义的一部分；改写为 PIVOT 标签会丢弃
// FORWARD/BACKWARD 信息并阻断同向合并。典型真实场景是泊车最后一次原地
// 小角度修正对齐车位：该段必须保留"前进/后退对齐"的真实方向标签，
// 且不得因重分类而脱离与相邻同向段的合并。
TEST(MincoThetaSManeuverMelterTest, FirstAndLastManeuverNeverPivot) {
    const FabricatedScene scene = Fabricate({
...
TEST(MincoThetaSManeuverMelterTest, MultiSegmentManeuverMeasuredAcrossSegments) {
    const FabricatedScene scene = Fabricate({
...
// 明确拒绝。
TEST(MincoThetaSManeuverMelterTest, InvalidInputsThrow) {
    const FabricatedScene scene = Fabricate({
        {Direction::FORWARD, 0.0, 1.0, 2.0},
        {Direction::BACKWARD, 0.01, -0.01, 1.0},
        {Direction::FORWARD, 0.0, 1.0, 2.0},
    ...
    EXPECT_THROW(melter.meltAndPrune(scene.trajectory, {MincoThetaSManeuverEstimate{}},
                                     {0.0, 0.0}, kinematics),
                 std::invalid_argument);
    // 段数不一致：估计 2 段 vs 轨迹 3 段
    std::vector<MincoThetaSManeuverEstimate> mismatched = scene.estimates;
    mismatched.pop_back();
...
TEST(MincoThetaSManeuverMelterTest, InvalidConfigThrows) {
    EXPECT_NO_THROW(const MincoThetaSManeuverMelter valid_melter(MincoThetaSConfig{}));
    MincoThetaSConfig config;
    config.melt_arc_threshold = 0.0;
    EXPECT_THROW(const MincoThetaSManeuverMelter m(config), std::invalid_argument);
    config = {};
...
TEST(MincoThetaSManeuverMelterTest, ConfigAccessorReturnsConstructionValues) {
    MincoThetaSConfig config;
...
TEST(MincoThetaSManeuverMelterTest, OutputManeuversMatchPhysicalDirectionRuns) {
    ...
    ResegmentByVelocityDirection(&path, MincoThetaSConfig{}.v_epsilon);
    ...
    for (const auto& m : path.getManeuvers()) {
        for (std::size_t i = 1; i < m.points.size(); ++i) {
            ...
            if (!p.hasV() || std::abs(p.getV()) < MincoThetaSConfig{}.v_epsilon) {
                continue;  // 停驻点不判方向
            }

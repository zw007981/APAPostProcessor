...
#include "core/MINCO_THETA_S/minco_theta_s_maneuver_segmenter.h"
#include "core/MINCO_THETA_S/minco_theta_s_preprocessor.h"
#include "core/MINCO_THETA_S/minco_theta_s_solver.h"
...
class MincoThetaSSolverTestAccessor : public MincoThetaSSolver {
   public:
    using MincoThetaSSolver::MincoThetaSSolver;
    using MincoThetaSSolver::buildProblem;
    using MincoThetaSSolver::evaluateCostAndGradient;
};
...
// 符号距离场严格为 D=x、梯度 g=(1,0)（与 minco_theta_s_esdf_penalty.t.cpp 同一场景
...
MincoThetaSConfig MakeKinematicsConfig() {
...
MincoThetaSPreprocessorResult RunPreprocessor(
    const Path& path, const Eigen::Vector2d& start_position,
    std::vector<MincoThetaSManeuverEstimate>* estimates) {
    MincoThetaSConfig segmenter_config;
    segmenter_config.nominal_segment_length = 0.6;
    *estimates = MincoThetaSManeuverSegmenter(segmenter_config).segment(path);
    const MincoThetaSPreprocessor preprocessor(MakeKinematicsConfig());
    const MincoThetaSPreprocessorResult result =
...
TEST(MincoThetaSSolverTest, StraightLineConvergesEndToEnd) {
    ...
    std::vector<MincoThetaSManeuverEstimate> estimates;
    const MincoThetaSPreprocessorResult pre_result =
        RunPreprocessor(BuildStraightPath(2.0), {0.0, 0.0}, &estimates);
    const MincoThetaSSolver solver(MakeKinematicsConfig());
    const MincoThetaSSolverResult result =
        solver.solve(estimates, pre_result, {0.0, 0.0}, esdf_map, footprint);
    EXPECT_TRUE(result.success);
    EXPECT_EQ(result.status, MincoThetaSSolverStatus::CONVERGED);
...
// 因为换挡点 ṡ=0 在 MINCO_THETA_S 框架内只能以软惩罚近似，收敛后换挡点残余速度
// 必须被压到接近 0 的工程容差内，同时终点双指标满足收敛判据。
TEST(MincoThetaSSolverTest, GearShiftConvergesEndToEnd) {
    const GridMap grid_map = BuildEmptyGridMap();
    const ESDFMap esdf_map(grid_map);
    const VehicleParams veh_params(4.3, 1.8, 2.7, 0.6, 0.8);
    const VehicleFootprintModel footprint = BuildFootprintModel(veh_params);
...
// 标定公式（MINCO_THETA_S.md 2.5 节）：ρ^0 = clip(J_s' / max(||C_f||², ε_ρ),
// ρ_min, ρ_max)，其中 J_s' 与 C_f 均取首轮内层收敛后的值。把收敛阈值压到
// 数值上不可达的 1e-300 强制跑满外层迭代，用 history 第 0 轮记录手推期望值。
TEST(MincoThetaSSolverTest, RhoInitialCalibrationMatchesHandDerived) {
    const GridMap grid_map = BuildEmptyGridMap();
    const ESDFMap esdf_map(grid_map);
    const VehicleParams veh_params(4.3, 1.8, 2.7, 0.6, 0.8);
    const VehicleFootprintModel footprint = BuildFootprintModel(veh_params);
    ...
    const MincoThetaSPreprocessorResult pre_result =
        RunPreprocessor(BuildStraightPath(2.0), {0.0, 0.0}, &estimates);
    MincoThetaSConfig config = MakeKinematicsConfig();
    ...
    const MincoThetaSSolver solver(config);
    const MincoThetaSSolverResult result =
        solver.solve(estimates, pre_result, {0.0, 0.0}, esdf_map, footprint);
    ASSERT_EQ(result.history.size(), 4U);
    const MincoThetaSOuterIterationRecord& round0 = result.history.front();
    ...
    const MincoThetaSOuterIterationRecord& round1 = result.history[1];
...
TEST(MincoThetaSSolverTest, MultiplierAndRhoUpdatesMatchFormula) {
    const GridMap grid_map = BuildEmptyGridMap();
    const ESDFMap esdf_map(grid_map);
    const VehicleParams veh_params(4.3, 1.8, 2.7, 0.6, 0.8);
    const VehicleFootprintModel footprint = BuildFootprintModel(veh_params);
    ...
    for (std::size_t k = 2; k < result.history.size(); ++k) {
        const MincoThetaSOuterIterationRecord& prev = result.history[k - 1];
        const MincoThetaSOuterIterationRecord& curr = result.history[k];
...
TEST(MincoThetaSSolverTest, RhoIncreaseGateHoldsWhenEnabled) {
    const GridMap grid_map = BuildEmptyGridMap();
    const ESDFMap esdf_map(grid_map);
    const VehicleParams veh_params(4.3, 1.8, 2.7, 0.6, 0.8);
    const VehicleFootprintModel footprint = BuildFootprintModel(veh_params);
...
TEST(MincoThetaSSolverTest, MaxOuterIterationsReturnsExplicitFailure) {
    const GridMap grid_map = BuildEmptyGridMap();
    const ESDFMap esdf_map(grid_map);
    const VehicleParams veh_params(4.3, 1.8, 2.7, 0.6, 0.8);
    const VehicleFootprintModel footprint = BuildFootprintModel(veh_params);
    ...
    EXPECT_EQ(result.status, MincoThetaSSolverStatus::MAX_OUTER_ITERATIONS);
...
// 测试含 MincoThetaSEsdfPenalty 的简单有障碍场景：收敛轨迹不侵入 margin_safe 边界。
...
TEST(MincoThetaSSolverTest, ObstacleSceneRespectsSafeMargin) {
    ...
    EXPECT_GE(min_clearance, MincoThetaSConfig{}.margin_safe - 1e-9);
}
...
TEST(MincoThetaSSolverTest, AnalyticGradientMatchesFiniteDifference) {
    const GridMap grid_map = BuildWallGridMap();
    const ESDFMap esdf_map(grid_map);
    const VehicleParams veh_params(4.3, 1.8, 2.7, 0.6, 0.8);
    const VehicleFootprintModel footprint = BuildFootprintModel(veh_params);
    ...
    std::vector<MincoThetaSManeuverEstimate> estimates(2);
    ...
    MincoThetaSPreprocessorResult pre_result;
    ...
    const MincoThetaSSolverTestAccessor solver(config);
    ...
    const MincoThetaSEsdfPenalty penalty(esdf_map, footprint, MincoThetaSConfig{});
    // 非零乘子/权重，使 MINCO_THETA_S 终端项的值与梯度都处于激活区
    const MincoThetaSMultiplierState multipliers{0.4, -0.3, 37.0};
    ...
    EXPECT_GT(breakdown.minco_theta_s_terminal, 0.0);
    for (int k = 0; k < x0.size(); ++k) {
        ...
        MincoThetaSCostBreakdown scratch;
...
TEST(MincoThetaSSolverTest, InvalidInputsThrow) {
    const GridMap grid_map = BuildEmptyGridMap();
    const ESDFMap esdf_map(grid_map);
    const VehicleParams veh_params(4.3, 1.8, 2.7, 0.6, 0.8);
    const VehicleFootprintModel footprint = BuildFootprintModel(veh_params);
    ...
    const MincoThetaSPreprocessorResult pre_result =
        RunPreprocessor(BuildStraightPath(2.0), {0.0, 0.0}, &estimates);
    const MincoThetaSSolver solver(MincoThetaSConfig{});
    // 空估计
...
TEST(MincoThetaSSolverTest, InvalidConfigThrows) {
    EXPECT_NO_THROW(const MincoThetaSSolver valid_solver(MincoThetaSConfig{}));
    MincoThetaSConfig config;
    config.terminal_position_tolerance = 0.0;
    EXPECT_THROW(const MincoThetaSSolver s(config), std::invalid_argument);
    config = {};
    ...
    EXPECT_NO_THROW(const MincoThetaSSolver s(config));
    config = {};
...
// 逐字段一致。
TEST(MincoThetaSSolverTest, ConfigAccessorReturnsConstructionValues) {
    MincoThetaSConfig config;

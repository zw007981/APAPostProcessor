...
#include "scene/ilqr_planning_scene.h"
...
TEST(iLQRPlanningSceneTest, FactoryRoutesToiLQRSceneWithConfiguredOuterRowNum) {
    ...
    EXPECT_EQ(scene->algorithmName(), "iLQR");
    EXPECT_EQ(scene->footprintModel().getOuterRowNum(), 3);
}

// 测试场景：详情 JSON 配置 iLQR 专有字段（reference/solver.outer 节）。
// 预期行为：专有字段经 loadConfigDetails 生效，且 reference 节的幅值键
// 经单一来源同步进 cost 同源字段。
TEST(iLQRPlanningSceneTest, AppliesiLQRSpecificOverrides) {
...
TEST(iLQRPlanningSceneTest, KeepsiLQRDefaultsWhenDetailsAbsent) {
    auto scene = LoadSceneWithDetails("{\"algorithm\": \"ilqr\"}");
    ASSERT_NE(scene, nullptr);
    const auto* ilqr_scene = dynamic_cast<const ILQRPlanningScene*>(scene.get());
    ASSERT_NE(ilqr_scene, nullptr);
...
TEST(iLQRPlanningSceneTest, MissingDetailsFileKeepsDefaults) {
    auto scene = LoadSceneWithDetails("{\"algorithm\": \"ilqr\"}");
    ASSERT_NE(scene, nullptr);
    auto* ilqr_scene = dynamic_cast<ILQRPlanningScene*>(scene.get());
    ASSERT_NE(ilqr_scene, nullptr);
    ASSERT_NO_THROW(
...
// iLQR 场景同样生效）。
TEST(iLQRPlanningSceneTest, OptimizeSummaryHandlesNotExecuted) {
    auto scene = LoadSceneWithDetails("{\"algorithm\": \"ilqr\"}");
    ASSERT_NE(scene, nullptr);

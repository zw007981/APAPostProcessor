...
static_assert(std::is_copy_constructible_v<MincoThetaSConfig>);
static_assert(std::is_copy_assignable_v<MincoThetaSConfig>);
static_assert(std::is_move_constructible_v<MincoThetaSConfig>);
...
TEST(ConfigTest, DerivedConfigsRemainCopyable) {
    ...
    MincoThetaSConfig minco_theta_s_src;
    minco_theta_s_src.max_velocity = 1.5;
    const MincoThetaSConfig minco_theta_s_dst = minco_theta_s_src;
    EXPECT_DOUBLE_EQ(minco_theta_s_dst.max_velocity, 1.5);
}

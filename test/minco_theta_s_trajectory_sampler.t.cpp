...
#include "core/MINCO_THETA_S/minco_theta_s_trajectory_sampler.h"
...
TEST(MincoThetaSTrajectorySamplerTest, SingleManeuverSamplingMatchesTrajectory) {
    ...
    const std::vector<Maneuver> maneuvers =
        SampleMincoThetaSTrajectory(scene.trajectory, scene.estimates, {1.0, 2.0},
    ...
    // 可行性校验（梯形配点残差）才能对 MINCO_THETA_S 产出真正生效
...
TEST(MincoThetaSTrajectorySamplerTest, GearShiftManeuverIntegratesSignedArcLength) {
    const FabricatedScene scene = Fabricate({
        {Direction::FORWARD, 0.0, 1.0, 2.0},
    ...
    const BicycleKinematicsExtractor kinematics = MakeKinematics();
    const std::vector<Maneuver> maneuvers =
        SampleMincoThetaSTrajectory(scene.trajectory, scene.estimates, {0.0, 0.0},
                              kinematics, kSamplesPerSegment);
...
TEST(MincoThetaSTrajectorySamplerTest, MultiManeuverCountsAndDirectionsPreserved) {
    const FabricatedScene scene = Fabricate({
        {Direction::FORWARD, 0.0, 1.0, 2.0},
        {Direction::BACKWARD, 0.0, -0.5, 1.5},
        {Direction::FORWARD, 0.0, 1.0, 2.0},
...
// 离散化两条段结构一致但数值不同的 MincoThetaSTrajectory，两次结果各自与对应
// 轨迹一致；对同一条轨迹重复离散化必须逐位一致（纯函数语义）。
TEST(MincoThetaSTrajectorySamplerTest, RepeatedCallsArePureAndIndependent) {
    ...
    const std::vector<Maneuver> result_a1 =
        SampleMincoThetaSTrajectory(scene_a.trajectory, scene_a.estimates, {0.0, 0.0},
                              kinematics, kSamplesPerSegment);
    const std::vector<Maneuver> result_b =
        SampleMincoThetaSTrajectory(scene_b.trajectory, scene_a.estimates, {0.0, 0.0},
                              kinematics, kSamplesPerSegment);
...
TEST(MincoThetaSTrajectorySamplerTest, InvalidInputsThrow) {
    const FabricatedScene scene = Fabricate({
        {Direction::FORWARD, 0.0, 1.0, 2.0},
        {Direction::BACKWARD, 0.0, -0.5, 1.5},
    });
    const BicycleKinematicsExtractor kinematics = MakeKinematics();
    EXPECT_NO_THROW(SampleMincoThetaSTrajectory(scene.trajectory, scene.estimates,
                                          {0.0, 0.0}, kinematics,
                                          kSamplesPerSegment));
    // 空估计
    EXPECT_THROW(SampleMincoThetaSTrajectory(scene.trajectory, {}, {0.0, 0.0},
                                       kinematics, kSamplesPerSegment),
                 std::invalid_argument);
    // 空 segments 的 Maneuver
    EXPECT_THROW(
        SampleMincoThetaSTrajectory(scene.trajectory, {MincoThetaSManeuverEstimate{}},
    ...
    mismatched.pop_back();
    EXPECT_THROW(SampleMincoThetaSTrajectory(scene.trajectory, mismatched, {0.0, 0.0},
                                       kinematics, kSamplesPerSegment),
                 std::invalid_argument);
    // 非有限起点世界坐标
    EXPECT_THROW(
        SampleMincoThetaSTrajectory(scene.trajectory, scene.estimates,
    ...
    EXPECT_THROW(
        ...
        std::invalid_argument);
    // 非法采样数
    EXPECT_THROW(SampleMincoThetaSTrajectory(scene.trajectory, scene.estimates,
...
TEST(MincoThetaSTrajectorySamplerTest, StructureCheckRejectsInconsistentEstimates) {
    const FabricatedScene scene = Fabricate({
        {Direction::FORWARD, 0.0, 1.0, 2.0},
        {Direction::BACKWARD, 0.0, -0.5, 1.5},
    });
    EXPECT_NO_THROW(
        CheckMincoThetaSSampleStructure(scene.trajectory, scene.estimates));
    EXPECT_THROW(CheckMincoThetaSSampleStructure(scene.trajectory, {}),
                 std::invalid_argument);
    EXPECT_THROW(
        CheckMincoThetaSSampleStructure(scene.trajectory, {MincoThetaSManeuverEstimate{}}),
        std::invalid_argument);
    std::vector<MincoThetaSManeuverEstimate> mismatched = scene.estimates;
    mismatched.pop_back();
    EXPECT_THROW(CheckMincoThetaSSampleStructure(scene.trajectory, mismatched),
                 std::invalid_argument);
}

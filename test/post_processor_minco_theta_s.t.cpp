...
class PostProcessorMincoThetaSTestAccess : public PostProcessor {
   public:
    using PostProcessor::PostProcessor;
    static MincoThetaSConfig CallDeriveKinematicsConfig(
        const VehicleParams& vehicle_params, const MincoThetaSConfig& minco_theta_s_config) {
        return DeriveKinematicsConfig(vehicle_params, minco_theta_s_config);
    }
};
...
// max_accel 与 |max_decel| 的较小值；速度上限取自 MINCO_THETA_S 配置。
TEST(PostProcessorMincoThetaSTest, DeriveKinematicsConfigMatchesVehicleParams) {
    const auto vehicle_params = MakeVehicleParams();
    MincoThetaSConfig minco_theta_s_config;
    minco_theta_s_config.max_velocity = 1.8;
    const auto kinematics_config =
        PostProcessorMincoThetaSTestAccess::CallDeriveKinematicsConfig(vehicle_params,
                                                                minco_theta_s_config);
...
// ============================================================
// 测试：MINCO_THETA_S 路径不污染配置、不与 NMPC 路径互相干扰
// ============================================================

// 调用方传入的 MincoThetaSConfig 对象在 optimizeMincoThetaS 后必须保持原值。
TEST(PostProcessorMincoThetaSTest, DoesNotMutateMincoThetaSConfig) {
    const auto vehicle_params = MakeVehicleParams();
    ...
    MincoThetaSConfig minco_theta_s_config;
    minco_theta_s_config.nominal_segment_length = 0.45;
    minco_theta_s_config.max_outer_iterations = 3;
    minco_theta_s_config.solver_weight_gear_cusp = 500.0;
    minco_theta_s_config.margin_safe = 0.03;
    minco_theta_s_config.melt_arc_threshold = 0.08;
    minco_theta_s_config.max_velocity = 1.5;
    const auto path = BuildStraightPath(2.0);
    processor.optimizeMincoThetaS(path, minco_theta_s_config);
    EXPECT_DOUBLE_EQ(minco_theta_s_config.nominal_segment_length, 0.45);
    EXPECT_EQ(minco_theta_s_config.max_outer_iterations, 3);
    EXPECT_DOUBLE_EQ(minco_theta_s_config.solver_weight_gear_cusp, 500.0);
    EXPECT_DOUBLE_EQ(minco_theta_s_config.margin_safe, 0.03);
    EXPECT_DOUBLE_EQ(minco_theta_s_config.melt_arc_threshold, 0.08);
    EXPECT_DOUBLE_EQ(minco_theta_s_config.max_velocity, 1.5);
}

// 先跑一次 NMPC 路径（max_iter=0 强制回退），再跑 MINCO_THETA_S 路径，最后再跑
// NMPC 路径：两次 NMPC 结果必须完全一致，且 NMPC 配置对象未被触碰。
TEST(PostProcessorMincoThetaSTest, MincoThetaSPathDoesNotInterfereWithNmpcPath) {
    const auto vehicle_params = MakeVehicleParams();
    const auto footprint = MakeFootprintModel(vehicle_params);
    const auto esdf_map = MakeLargeEmptyEsdfMap();
    const PostProcessor processor(vehicle_params, footprint, esdf_map);
    ...
    const auto minco_theta_s_result = processor.optimizeMincoThetaS(path, MincoThetaSConfig{});
    ASSERT_TRUE(minco_theta_s_result.success) << minco_theta_s_result.message;
    ...
    // NMPC 配置对象未被 MINCO_THETA_S 路径触碰
    EXPECT_EQ(nmpc_config.max_iter, 0);
    EXPECT_FALSE(nmpc_config.use_static_corridor);
}

// ============================================================
// 测试：minco_theta_s_traj 结果轨迹填充
// ============================================================

// 成功路径必须同步填充 minco_theta_s_traj（点数与输出 Path 一致，逐点携带状态/控制量）；
// 失败路径 minco_theta_s_traj 必须为空。
TEST(PostProcessorMincoThetaSTest, MincoThetaSTrajFilledOnSuccessAndEmptyOnFailure) {
    const auto vehicle_params = MakeVehicleParams();
    const auto footprint = MakeFootprintModel(vehicle_params);
    const auto esdf_map = MakeLargeEmptyEsdfMap();
    const PostProcessor processor(vehicle_params, footprint, esdf_map);
    const auto path = BuildStraightPath(2.0);
    const auto result = processor.optimizeMincoThetaS(path, MincoThetaSConfig{});
    ...
    const auto failed = processor.optimizeMincoThetaS(empty_path, MincoThetaSConfig{});
    EXPECT_FALSE(failed.success);
    EXPECT_TRUE(failed.optimized_trajectory.empty());
}

// ============================================================
// 测试：minco_theta_s_preprocessed_traj 预处理粗优化轨迹填充
// ============================================================

// 预处理成功后必须填充 intermediate_traces["minco_theta_s_preprocessed"]（预处理
// 粗优化轨迹的离散化结果，作为"优化前"对比基线）：首点锚定初始路径首点，
// 末点在预处理收敛容差内贴近初始路径末点，逐点携带状态/控制量；预处理
// 失败时 intermediate_traces 中无此项。
TEST(PostProcessorMincoThetaSTest,
     MincoThetaSPreprocessedTrajFilledOnSuccessAndEmptyOnFailure) {
    const auto vehicle_params = MakeVehicleParams();
    const auto footprint = MakeFootprintModel(vehicle_params);
    const auto esdf_map = MakeLargeEmptyEsdfMap();
    const PostProcessor processor(vehicle_params, footprint, esdf_map);
    ...
    for (const auto& [name, traj] : result.intermediate_traces) {
        if (name == "minco_theta_s_preprocessed") {
            preprocessed = &traj;
            break;
        }
    }
    ...
    // 预处理失败：intermediate_traces 中无 minco_theta_s_preprocessed 项
    MincoThetaSConfig fail_config;
    fail_config.convergence_position_tolerance = 1e-9;
    const auto failed = processor.optimizeMincoThetaS(path, fail_config);
...
TEST(PostProcessorMincoThetaSTest, EmptyPathReturnsFailure) {
    const auto vehicle_params = MakeVehicleParams();
    const auto footprint = MakeFootprintModel(vehicle_params);
    const auto esdf_map = MakeLargeEmptyEsdfMap();
    const PostProcessor processor(vehicle_params, footprint, esdf_map);
    const Path empty_path;
    const auto result = processor.optimizeMincoThetaS(empty_path, MincoThetaSConfig{});
...
TEST(PostProcessorMincoThetaSTest, PreprocessFailureReturnsFailure) {
    const auto vehicle_params = MakeVehicleParams();
    const auto footprint = MakeFootprintModel(vehicle_params);
    const auto esdf_map = MakeLargeEmptyEsdfMap();
    const PostProcessor processor(vehicle_params, footprint, esdf_map);
    MincoThetaSConfig minco_theta_s_config;
    minco_theta_s_config.convergence_position_tolerance = 1e-9;
    const auto path = BuildStraightPath(2.0);
    const auto result = processor.optimizeMincoThetaS(path, minco_theta_s_config);
    EXPECT_FALSE(result.success);
    EXPECT_TRUE(result.optimized_path.empty());
...
// 空旷直线场景应收敛：终点双指标满足 MINCO_THETA_S 收敛判据，轨迹无碰撞、无奇异。
TEST(PostProcessorMincoThetaSTest, EndToEndStraightLineConverges) {
    const auto vehicle_params = MakeVehicleParams();
    const auto footprint = MakeFootprintModel(vehicle_params);
    const auto esdf_map = MakeLargeEmptyEsdfMap();
    const PostProcessor processor(vehicle_params, footprint, esdf_map);
...
TEST(PostProcessorMincoThetaSTest, EndToEndGearShiftConverges) {
    const auto vehicle_params = MakeVehicleParams();
    const auto footprint = MakeFootprintModel(vehicle_params);
    const auto esdf_map = MakeLargeEmptyEsdfMap();
    const PostProcessor processor(vehicle_params, footprint, esdf_map);
...
TEST(PostProcessorMincoThetaSTest, OptimizedTrajectoryContainsFullKinematics) {
    const auto vehicle_params = MakeVehicleParams();
    const auto footprint = MakeFootprintModel(vehicle_params);
    const auto esdf_map = MakeLargeEmptyEsdfMap();
    const PostProcessor processor(vehicle_params, footprint, esdf_map);
...
TEST(PostProcessorMincoThetaSTest, AlgorithmFieldIsMincoThetaS) {
    const auto vehicle_params = MakeVehicleParams();
    const auto footprint = MakeFootprintModel(vehicle_params);
    const auto esdf_map = MakeLargeEmptyEsdfMap();
    const PostProcessor processor(vehicle_params, footprint, esdf_map);
    ...
    EXPECT_EQ(result.algorithm, "minco_theta_s");
}

// intermediate_traces 包含预处理轨迹（成功路径）
TEST(PostProcessorMincoThetaSTest, IntermediateTracesContainMincoThetaSPreprocessed) {
    const auto vehicle_params = MakeVehicleParams();
    const auto footprint = MakeFootprintModel(vehicle_params);
    const auto esdf_map = MakeLargeEmptyEsdfMap();
    const PostProcessor processor(vehicle_params, footprint, esdf_map);
    ...
    const auto result = processor.optimizeMincoThetaS(path, MincoThetaSConfig{});
    ASSERT_TRUE(result.success) << result.message;
    // intermediate_traces 应包含 "minco_theta_s_preprocessed" 条目
    bool found = false;
    for (const auto& [name, traj] : result.intermediate_traces) {
        if (name == "minco_theta_s_preprocessed") {
            found = true;
    ...
    EXPECT_TRUE(found) << "intermediate_traces should contain minco_theta_s_preprocessed";
}

// output_level 分级语义
TEST(PostProcessorMincoThetaSTest, OutputLevelReflectsResultQuality) {
    const auto vehicle_params = MakeVehicleParams();
    const auto footprint = MakeFootprintModel(vehicle_params);
    const auto esdf_map = MakeLargeEmptyEsdfMap();
    const PostProcessor processor(vehicle_params, footprint, esdf_map);

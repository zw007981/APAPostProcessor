...
TEST(MincoThetaSConfigTest, LoadFromJsonOverridesAllEsdfFields) {
    const auto details = nlohmann::json::parse(R"json({
        "esdf": {
...
TEST(MincoThetaSConfigTest, LoadFromJsonRejectsNullConfig) {
    const auto details =
        nlohmann::json::parse(R"json({"algorithm": "minco_theta_s"})json");
    EXPECT_THROW(LoadMincoThetaSConfigOverrides(details, nullptr),
...
TEST(MincoThetaSConfigTest, LoadFromJsonIgnoresUnknownFields) {
    const auto details = nlohmann::json::parse(R"json({
        "algorithm": "minco_theta_s",
...
// 作者笔误，必须显式失败而非静默纠错）
TEST(MincoThetaSConfigTest, LoadFromJsonThrowsOnWrongFieldType) {
    const auto details = nlohmann::json::parse(R"json({
        "esdf": {"margin_safe": "not_a_number"}
    })json");
    MincoThetaSConfig config;
    EXPECT_THROW(LoadMincoThetaSConfigOverrides(details, &config),

...
// 测试用车辆参数与外圆模型（与 iLQR 数据集同源：轴距 3.0 m）
...
// 起终点位姿逐位保持（前后缀不参与替换）
TEST(iLQRRsShortcutTest, DetourIsShortcutInFreeSpace) {
...
TEST(iLQRRsShortcutTest, ShortcutCurvatureRespectsCap) {
...
TEST(iLQRRsShortcutTest, CollidingShortcutIsRejected) {
    const Path input = BuildUTurnPath();
...
TEST(iLQRRsShortcutTest, LengthGrowthGuardBoundsOutputLength) {
    const Path input = BuildDetourPath();
    const ESDFMap map = MakeEmptyMap();
    const VehicleFootprintModel footprint = MakeFootprint();
...
// 「新机制默认关闭且关闭时位一致」的硬要求
TEST(iLQRRsShortcutTest, DisabledConfigPassesPathThrough) {
    const Path input = BuildDetourPath();
    const ESDFMap map = MakeEmptyMap();
    const VehicleFootprintModel footprint = MakeFootprint();
...
TEST(iLQRRsShortcutTest, InvalidConfigThrows) {
    const Path input = BuildDetourPath();
    const ESDFMap map = MakeEmptyMap();
    const VehicleFootprintModel footprint = MakeFootprint();
...
// 「换挡节点不得凭空变向」语义的载体
TEST(iLQRRsShortcutTest, RespectsNodeDepartureDirection) {
    const Path input = BuildDetourPath();
    const ESDFMap map = MakeEmptyMap();
    const VehicleFootprintModel footprint = MakeFootprint();
    const Path output = ShortcutShiftPoints(input, map, footprint, kWheelbase,
...
TEST(iLQRRsShortcutTest, ReconstructionPreservesTerminalPose) {

...
TEST_F(GridMapTest, BuildFromTestJsonEnvironmentSuccessfully) {
    ...
    EXPECT_EQ(grid_map.getOccupancyData().size(),
              static_cast<std::size_t>(40000));
}
...
TEST_F(GridMapTest,
       GetIndexByWorldReturnsMinusOneWhenOutOfBoundsOnTestJsonMap) {
    const GridMap grid_map = loadGridMapFromTestJson();
...
// 因为路径规划通常将地图外视为不可通行区域，所以越界查询应返回 occupied
// 以避免越界穿越。
...
// 因为内部占据数组大小依赖
// width*height，非正尺寸会导致地图语义无效，所以应在构造阶段拒绝。
...
// 测试空白区域映射为未占用。
// 因为碰撞检测若把空白区域误判为障碍会触发假阳性，所以必须验证典型自由空间坐标返回
// false。
...
// 因为 x/y 位于网格边界附近时最容易触发 floor
// 精度陷阱，所以这里验证边界稳定性和不越界行为。
...
// origin 本身即为第 0 行第 0 列栅格的中心，因此 getPosition(0, 0) 应严格等于
// origin。
...
// 因为栅格中心是 occupancy 的代表点，返回中心后查询 isOccupied 应与 data_
// 一致。

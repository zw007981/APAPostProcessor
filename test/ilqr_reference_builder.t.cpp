...
TEST(iLQRReferenceBuilderTest, StraightLineResamplesToUniformSpacing) {
    ...
    const iLQRReference reference = builder.build(path);
    ...
    odd_path.finalize();
    const iLQRReference odd_reference = builder.build(odd_path);
...
TEST(iLQRReferenceBuilderTest, ShiftPathProducesCuspsAndManeuverMetadata) {
    ...
    EXPECT_NEAR(reference.initial_states[19](ILQR_IDX_V), 0.5, 1e-9);
    EXPECT_NEAR(reference.initial_states[20](ILQR_IDX_V), -0.5, 1e-9);
    EXPECT_NEAR(reference.initial_states[33](ILQR_IDX_V), -0.5, 1e-9);
    EXPECT_NEAR(reference.initial_states[34](ILQR_IDX_V), 0.5, 1e-9);
    EXPECT_NEAR(reference.initial_states[44](ILQR_IDX_V), 0.5, 1e-9);
}
...
TEST(iLQRReferenceBuilderTest, StraightLineInitialGuessMatchesAnalyticValues) {
    Path path;
    path.addPoint({0.0, 0.0, 0.0});
    AppendXLine(&path, 0.0, 2.0, 0.0);
    path.finalize();
    ...
    for (const auto& state : reference.initial_states) {
        EXPECT_NEAR(state(ILQR_IDX_V), 0.5, 1e-12);
        EXPECT_NEAR(state(ILQR_IDX_A), 0.0, 1e-12);
        EXPECT_NEAR(state(ILQR_IDX_DELTA), 0.0, 1e-12);
        EXPECT_NEAR(state(ILQR_IDX_OMEGA), 0.0, 1e-12);
    }
...
TEST(iLQRReferenceBuilderTest, CircularArcInitialGuessMatchesAnalyticCurvature) {
    Path path;
    path.addPoint({0.0, 0.0, 0.0});
    ...
    for (const auto& state : reference.initial_states) {
        ...
        EXPECT_NEAR(state(ILQR_IDX_V), 0.5, 1e-6);
        EXPECT_NEAR(state(ILQR_IDX_A), 0.0, 1e-6);
        EXPECT_NEAR(state(ILQR_IDX_OMEGA), 0.0, 1e-6);
    }
    // 内部 knot 的 δ 与解析值一致（端点由单侧/拷贝差分得到，同样应一致）
    for (std::size_t k = 0; k < reference.initial_states.size(); ++k) {
        EXPECT_NEAR(reference.initial_states[k](ILQR_IDX_DELTA), expected_delta,
                    1e-6)
            << "k = " << k;
    }
}
...
TEST(iLQRReferenceBuilderTest, HeadingWrapAcrossPiHasNoSpuriousArtifacts) {
    Path path;
    for (int k = 0; k <= 40; ++k) {
    ...
    for (const auto& state : reference.initial_states) {
        EXPECT_NEAR(state(ILQR_IDX_DELTA), expected_delta, 1e-9);
        EXPECT_NEAR(state(ILQR_IDX_V), 0.5, 1e-9);
    }
}

// 四份真实数据集的验收测试：Path 均可成功预处理，输出规模与
// iLQR.md 2.1 节量级一致（N = round(L/0.05)、打靶节点约 N/n_s+|C|+1 个），
// cusp 数量与 Path 的方向反号边界一一对应，初值全部有限且在盒内。
class iLQRReferenceDatasetTest
    : public ::testing::TestWithParam<std::pair<std::string, std::string>> {};

TEST_P(iLQRReferenceDatasetTest, RealDatasetPreprocessesToExpectedScale) {
    ...
    std::cout << "[iLQR-REF] dataset=" << name << " N=" << expected_n
...
INSTANTIATE_TEST_SUITE_P(
    FourDatasets, iLQRReferenceDatasetTest,

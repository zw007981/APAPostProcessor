...
// 因为内部统一使用 remainder 主值域，所以超过一圈的输入应折叠回 [-PI, PI] 附近。
...
// 因为日志输出需要稳定字段名和两位小数精度，所以 toString 应输出 x、y、theta 的 JSON 文本。

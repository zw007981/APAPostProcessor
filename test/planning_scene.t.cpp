...
// 测试场景：详情 JSON 的 algorithm 字段为 "minco_theta_s" 且配置 outer_row_num=3。
// 预期行为：工厂路由到 MINCO_THETA_S 场景，外圆行数透传为 3。
TEST(PlanningSceneTest, FactoryRoutesToMincoThetaSSceneWithConfiguredOuterRowNum) {
    auto scene =
        LoadSceneWithDetails("{\"algorithm\": \"minco_theta_s\", \"outer_row_num\": 3}");
    ASSERT_NE(scene, nullptr);
    EXPECT_EQ(scene->algorithmName(), "MINCO_THETA_S");
...
// MINCO_THETA_S 场景同一解析入口，防止 proto 路由静默忽略基类字段）。
...
// 测试场景：MINCO_THETA_S 详情 JSON 未配置 outer_row_num。
// 预期行为：外圆行数取 Config 基类默认值 4。
TEST(PlanningSceneTest, OuterRowNumDefaultsToConfigDefaultWhenAbsent) {
    auto scene = LoadSceneWithDetails("{\"algorithm\": \"minco_theta_s\"}");
    ASSERT_NE(scene, nullptr);
    EXPECT_EQ(scene->footprintModel().getOuterRowNum(), 4);
}
...
// 测试场景：优化失败时生成优化摘要（data/test.json 的退化路径使 MINCO_THETA_S
// 预处理无法收敛，真实失败分支）。
...
// 测试场景：优化成功后生成优化摘要（data3 真实数据集，MINCO_THETA_S 可收敛）。

...
class MincoThetaSPreprocessorTestAccessor : public MincoThetaSPreprocessor {
   public:
    using MincoThetaSPreprocessor::MincoThetaSPreprocessor;
    using MincoThetaSPreprocessor::buildProblem;
    using MincoThetaSPreprocessor::evaluateCostAndGradient;
};
...
std::vector<MincoThetaSManeuverEstimate> MakeSimpleEstimates() {
    std::vector<MincoThetaSManeuverEstimate> estimates(1);
...
// 测试直线单机动场景下的收敛性。
// 因为初值来自 MincoThetaSManeuverSegmenter 的真实解析结果且场景无障碍无转弯，
// 所以松收敛阈值下 L-BFGS 必须收敛，各段末端世界坐标必须贴近前端锚点，
// 终点硬边界（零速）必须精确满足。
TEST(MincoThetaSPreprocessorTest, StraightSingleManeuverConverges) {
    const MincoThetaSPreprocessor preprocessor(MincoThetaSConfig{});
    const Path path = BuildStraightPath(2.0);
    const std::vector<MincoThetaSManeuverEstimate> estimates =
        MincoThetaSManeuverSegmenter(MincoThetaSConfig{}).segment(path);
...
// 因为换挡点 ṡ=0 在 MINCO_THETA_S 框架内只能以软惩罚近似，所以收敛后换挡点残余
// 速度必须被压到接近 0 的工程容差内，且两段末端位置都要贴近各自锚点。
TEST(MincoThetaSPreprocessorTest, GearShiftTwoManeuverConverges) {
    const MincoThetaSPreprocessor preprocessor(MincoThetaSConfig{});
...
// 恒零，没有验证价值）。
TEST(MincoThetaSPreprocessorTest, AnalyticGradientMatchesFiniteDifference) {
    ...
    const MincoThetaSPreprocessorTestAccessor preprocessor(config);
...
TEST(MincoThetaSPreprocessorTest, DurationBalanceConstrainsSpread) {
    std::vector<MincoThetaSManeuverEstimate> estimates(1);
    estimates[0].direction = Direction::FORWARD;
    estimates[0].start_theta = 0.0;
    estimates[0].start_arc_length = 0.0;
...
// 的判据
TEST(MincoThetaSPreprocessorTest, ConflictingLimitsReturnFailure) {
    MincoThetaSConfig config;
    config.max_velocity = 1e-3;
    config.pre_weight_velocity = 1e9;
    config.pre_epsilon_time = 1.0;
    const MincoThetaSPreprocessor preprocessor(config);
    const Path path = BuildStraightPath(2.0);
    const std::vector<MincoThetaSManeuverEstimate> estimates =
        MincoThetaSManeuverSegmenter(MincoThetaSConfig{}).segment(path);
    const MincoThetaSPreprocessorResult result =
...
TEST(MincoThetaSPreprocessorTest, InvalidInputsThrow) {
    const MincoThetaSPreprocessor preprocessor(MincoThetaSConfig{});
    // 空估计
    EXPECT_THROW(preprocessor.preprocess({}, {0.0, 0.0}),
                 std::invalid_argument);
    // 空 segments 的 Maneuver
    EXPECT_THROW(preprocessor.preprocess({MincoThetaSManeuverEstimate{}}, {0.0, 0.0}),
                 std::invalid_argument);
    // 非有限起点世界坐标
...
TEST(MincoThetaSPreprocessorTest, InvalidConfigThrows) {
    EXPECT_NO_THROW(const MincoThetaSPreprocessor valid_preprocessor(MincoThetaSConfig{}));
    MincoThetaSConfig config;
    config.pre_weight_velocity = -1.0;
    EXPECT_THROW(const MincoThetaSPreprocessor p(config), std::invalid_argument);
    config = {};
    ...
    EXPECT_NO_THROW(const MincoThetaSPreprocessor p(config));
    config = {};
...
TEST(MincoThetaSPreprocessorTest, ConfigAccessorReturnsConstructionValues) {
    MincoThetaSConfig config;

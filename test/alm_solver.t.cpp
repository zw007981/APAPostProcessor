...
class AlmSolverTestAccessor : public AlmSolver {
   public:
    using AlmSolver::AlmSolver;
    using AlmSolver::buildProblem;
    using AlmSolver::evaluateCostAndGradient;
};
...
BicycleKinematicsConfig MakeKinematicsConfig() {
...
TEST(AlmSolverTest, StraightLineConvergesEndToEnd) {
    ...
    EXPECT_TRUE(result.success);
    EXPECT_EQ(result.status, AlmSolverStatus::CONVERGED);
...
TEST(AlmSolverTest, GearShiftConvergesEndToEnd) {
    const GridMap grid_map = BuildEmptyGridMap();
    const ESDFMap esdf_map(grid_map);
    const VehicleParams veh_params(4.3, 1.8, 2.7, 0.6, 0.8);
    const VehicleFootprintModel footprint = BuildFootprintModel(veh_params);
...
// 标定公式（ALM.md 2.5 节）：ρ^0 = clip(J_s' / max(||C_f||², ε_ρ),
// ρ_min, ρ_max)，其中 J_s' 与 C_f 均取首轮内层收敛后的值。把收敛阈值压到
// 数值上不可达的 1e-300 强制跑满外层迭代，用 history 第 0 轮记录手推期望值。
TEST(AlmSolverTest, RhoInitialCalibrationMatchesHandDerived) {
    const GridMap grid_map = BuildEmptyGridMap();
    const ESDFMap esdf_map(grid_map);
    const VehicleParams veh_params(4.3, 1.8, 2.7, 0.6, 0.8);
    const VehicleFootprintModel footprint = BuildFootprintModel(veh_params);
    ...
    const AlmSolver solver(config, MakeKinematicsConfig());
    const AlmSolverResult result =
        solver.solve(estimates, pre_result, {0.0, 0.0}, esdf_map, footprint);
    ASSERT_EQ(result.history.size(), 4U);
    const AlmOuterIterationRecord& round0 = result.history.front();
    ...
    const AlmOuterIterationRecord& round1 = result.history[1];
...
TEST(AlmSolverTest, MultiplierAndRhoUpdatesMatchFormula) {
    const GridMap grid_map = BuildEmptyGridMap();
    const ESDFMap esdf_map(grid_map);
    const VehicleParams veh_params(4.3, 1.8, 2.7, 0.6, 0.8);
    const VehicleFootprintModel footprint = BuildFootprintModel(veh_params);
    ...
    for (std::size_t k = 2; k < result.history.size(); ++k) {
        const AlmOuterIterationRecord& prev = result.history[k - 1];
        const AlmOuterIterationRecord& curr = result.history[k];
...
TEST(AlmSolverTest, RhoIncreaseGateHoldsWhenEnabled) {
    const GridMap grid_map = BuildEmptyGridMap();
    const ESDFMap esdf_map(grid_map);
    const VehicleParams veh_params(4.3, 1.8, 2.7, 0.6, 0.8);
    const VehicleFootprintModel footprint = BuildFootprintModel(veh_params);
...
TEST(AlmSolverTest, MaxOuterIterationsReturnsExplicitFailure) {
    const GridMap grid_map = BuildEmptyGridMap();
    const ESDFMap esdf_map(grid_map);
    const VehicleParams veh_params(4.3, 1.8, 2.7, 0.6, 0.8);
    const VehicleFootprintModel footprint = BuildFootprintModel(veh_params);
...
// 测试含 AlmEsdfPenalty 的简单有障碍场景：收敛轨迹不侵入 margin_safe 边界。
...
TEST(AlmSolverTest, ObstacleSceneRespectsSafeMargin) {
    ...
    EXPECT_GE(min_clearance, AlmEsdfPenaltyConfig{}.margin_safe - 1e-9);
}
...
TEST(AlmSolverTest, AnalyticGradientMatchesFiniteDifference) {
    ...
    BicycleKinematicsConfig kinematics_config = MakeKinematicsConfig();
    kinematics_config.max_velocity = 0.25;
    kinematics_config.max_acceleration = 0.3;
    kinematics_config.max_steer_angle = 0.5;
    kinematics_config.max_steer_rate = 0.15;
    AlmSolverConfig config;
    ...
    // 非零乘子/权重，使 ALM 终端项的值与梯度都处于激活区
    const AlmMultiplierState multipliers{0.4, -0.3, 37.0};
    ...
    EXPECT_GT(breakdown.alm_terminal, 0.0);
...
TEST(AlmSolverTest, InvalidInputsThrow) {
    const GridMap grid_map = BuildEmptyGridMap();
    const ESDFMap esdf_map(grid_map);
    const VehicleParams veh_params(4.3, 1.8, 2.7, 0.6, 0.8);
    const VehicleFootprintModel footprint = BuildFootprintModel(veh_params);
...
TEST(AlmSolverTest, InvalidConfigThrows) {
    EXPECT_NO_THROW(const AlmSolver valid_solver);
    AlmSolverConfig config;
    config.terminal_position_tolerance = 0.0;
    EXPECT_THROW(const AlmSolver s(config), std::invalid_argument);
    config = {};

...
#include "core/MINCO_THETA_S/minco_theta_s_esdf_penalty.h"
...
MincoThetaSEsdfPoseCost ComputeWithFullQuery(
    const ESDFMap& esdf_map,
    const std::vector<Eigen::Vector2d>& local_centers, double radius, double x,
    double y, double theta, const MincoThetaSConfig& config) {
    MincoThetaSEsdfPoseCost result;
...
double NumericGradientComponent(const MincoThetaSEsdfPenalty& penalty, double x,
...
class MincoThetaSEsdfPenaltyTest : public ::testing::Test {
   protected:
    MincoThetaSEsdfPenaltyTest()
    ...
    MincoThetaSEsdfPenalty MakePenalty(MincoThetaSConfig config = {}) const {
        return MincoThetaSEsdfPenalty(esdf_map_, footprint_model_, config);
    }
...
TEST_F(MincoThetaSEsdfPenaltyTest, ConstructorThrowsOnInvalidConfig) {
...
TEST_F(MincoThetaSEsdfPenaltyTest, ReturnsZeroOutsideComfortMargin) {
...
TEST_F(MincoThetaSEsdfPenaltyTest, BoundaryAtComfortMarginIsContinuous) {
    const auto penalty = MakePenalty();
...
// 段单独起作用"，必须独立验证。
TEST_F(MincoThetaSEsdfPenaltyTest, HandDerivedComfortOnlyZoneMatches) {
    const auto penalty = MakePenalty();
...
TEST_F(MincoThetaSEsdfPenaltyTest, HandDerivedMixedZoneMatches) {
    const auto penalty = MakePenalty();
...
TEST_F(MincoThetaSEsdfPenaltyTest, HandDerivedRotatedPoseMatches) {
    const auto penalty = MakePenalty();
...
// 配置同样必须满足解析公式。
TEST_F(MincoThetaSEsdfPenaltyTest, CustomWeightsAndMarginsMatchHandDerived) {
    MincoThetaSConfig config;
...
// 混合段。
TEST_F(MincoThetaSEsdfPenaltyTest, PenaltyMatchesFullQueryReferenceBitExact) {
    const auto penalty = MakePenalty();
    ...
    for (const auto& pose : poses) {
        const MincoThetaSEsdfPoseCost gated =
            penalty.evaluate(pose[0], pose[1], pose[2]);
        const MincoThetaSEsdfPoseCost full =
...
TEST_F(MincoThetaSEsdfPenaltyTest, PrecomputedTrigEntryMatchesThetaEntryBitExact) {
    const auto penalty = MakePenalty();
    const std::array<std::array<double, 3>, 3> poses = {{
        {circle_radius_ + 0.5, 2.5, kHalfPi},
        {circle_radius_ + 0.06, 2.5, kHalfPi},
    ...
    for (const auto& pose : poses) {
        const MincoThetaSEsdfPoseCost via_theta =
            penalty.evaluate(pose[0], pose[1], pose[2]);
        const MincoThetaSEsdfPoseCost via_trig = penalty.evaluate(
...
TEST_F(MincoThetaSEsdfPenaltyTest, HandDerivedNegativeAngleMatches) {
    const auto penalty = MakePenalty();
...
TEST_F(MincoThetaSEsdfPenaltyTest, DeeperIntrusionYieldsStrictlyLargerCost) {
    const auto penalty = MakePenalty();
...
TEST_F(MincoThetaSEsdfPenaltyTest, NumericGradientMatchesAnalyticParallel) {
    const auto penalty = MakePenalty();
    const double x0 = 0.35;
    const auto result = penalty.evaluate(x0, 4.0, kHalfPi);
...
TEST_F(MincoThetaSEsdfPenaltyTest, NumericGradientMatchesAnalyticRotated) {
    const auto penalty = MakePenalty();
    const double theta = std::atan2(0.8, -0.6);
    const double x0 = 1.95;
    const auto result = penalty.evaluate(x0, 4.0, theta);
...
// 对应的全侵入罚）。
TEST_F(MincoThetaSEsdfPenaltyTest, OutOfMapCircleYieldsConservativePenalty) {
    const auto penalty = MakePenalty();
...
// 都必须保持本测试逐位通过。
TEST_F(MincoThetaSEsdfPenaltyTest, EvaluateMatchesPerCircleFullEvaluationBitExact) {
    const MincoThetaSConfig config;
    const auto penalty = MakePenalty(config);
    ...
    for (const auto& pose : poses) {
        ...
        // 前的实现逐条对应
        MincoThetaSEsdfPoseCost expected;
        for (const auto& center : circle_centers_) {
            const double lx = center.x();
            const double ly = center.y();

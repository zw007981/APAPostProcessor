...
ReferenceCostGradHess ComputeReference(
    ...
    double y, double theta, const DdpEsdfConstraintConfig& config) {
...
// 供约束雅可比的有限差分对拍使用
class TestableDdpEsdfConstraint : public DdpEsdfConstraint {
   public:
    using DdpEsdfConstraint::DdpEsdfConstraint;
    using DdpEsdfConstraint::evaluateCircle;
};

// 公共测试夹具：直墙 ESDF 地图 + 真实车辆外圆模型（outer_row_num=1，3 个外圆，
// 局部圆心 lx∈{-0.0833, 1.35, 2.7833}、ly=0，r_outer≈1.1505）
class DdpEsdfConstraintTest : public ::testing::Test {
   protected:
    DdpEsdfConstraintTest()
    ...
    DdpEsdfConstraint MakeConstraint(
        DdpEsdfConstraintConfig config = {}) const {
        return DdpEsdfConstraint(esdf_map_, footprint_model_, config);
    }

    TestableDdpEsdfConstraint MakeTestable(
        DdpEsdfConstraintConfig config = {}) const {
        return TestableDdpEsdfConstraint(esdf_map_, footprint_model_, config);
    }

    // 校验 evaluate 结果与手推参考一致（代价/梯度/Hessian 的 (x,y,θ) 块）
    void ExpectMatchesReference(const DdpEsdfPoseCost& actual,
                                const ReferenceCostGradHess& ref,
                                double rel_tol) const {
        ExpectComponentClose(ref.cost, actual.cost, rel_tol);
        ExpectComponentClose(ref.gradient.x(), actual.gradient(DDP_IDX_X),
                             rel_tol);
        EXPECT_NEAR(ref.gradient.y(), actual.gradient(DDP_IDX_Y), 1e-9);
        ExpectComponentClose(ref.gradient.z(), actual.gradient(DDP_IDX_THETA),
                             rel_tol);
...
TEST_F(DdpEsdfConstraintTest, ConstructorThrowsOnInvalidConfig) {
...
TEST_F(DdpEsdfConstraintTest, HandDerivedMixedZoneMatches) {
    const auto constraint = MakeConstraint();
...
TEST_F(DdpEsdfConstraintTest, HandDerivedRotatedPoseMatches) {
    const auto constraint = MakeConstraint();
    ...
    ExpectMatchesReference(result, ref, 1e-6);
    EXPECT_GT(std::abs(result.hessian(DDP_IDX_THETA, DDP_IDX_THETA)), 1.0);
}

// 测试平行位姿下解析梯度与五点中心差分数值梯度对拍，相对误差 < 1e-6。
// 数值梯度是独立于手推公式的第三方验证；场景取深惩罚区且差分模板
// 不跨越 max(0,C)³ 的 C=0 折点（折点主导差分误差，是已标定的坑）。
TEST_F(DdpEsdfConstraintTest, NumericGradientMatchesAnalyticParallel) {
    const auto constraint = MakeConstraint();
    const double x0 = 0.35;
    const auto result = constraint.evaluate(x0, 4.0, kHalfPi);
    ...
    ExpectComponentClose(result.gradient(DDP_IDX_X),
                         FivePointCentral(cost_of(0), 1e-3), 1e-6);
    EXPECT_NEAR(result.gradient(DDP_IDX_Y), FivePointCentral(cost_of(1), 1e-3),
                1e-9);
    ExpectComponentClose(result.gradient(DDP_IDX_THETA),
                         FivePointCentral(cost_of(2), 1e-3), 1e-6);
    // 非 (x,y,θ) 状态分量必须严格为零：惩罚只依赖位姿
    EXPECT_DOUBLE_EQ(result.gradient(DDP_IDX_V), 0.0);
    EXPECT_DOUBLE_EQ(result.gradient(DDP_IDX_A), 0.0);
    EXPECT_DOUBLE_EQ(result.gradient(DDP_IDX_DELTA), 0.0);
    EXPECT_DOUBLE_EQ(result.gradient(DDP_IDX_OMEGA), 0.0);
}

// 测试旋转位姿下解析梯度与数值梯度对拍，相对误差 < 1e-6（含 θ 通道）。
// 这是"若忽略外圆随 θ 旋转的链式法则，梯度会明显偏离数值梯度"的直接验证。
TEST_F(DdpEsdfConstraintTest, NumericGradientMatchesAnalyticRotated) {
    const auto constraint = MakeConstraint();
    const double theta = std::atan2(0.8, -0.6);
    const double x0 = 1.95;
    const auto result = constraint.evaluate(x0, 4.0, theta);
...
TEST_F(DdpEsdfConstraintTest, GnHessianMatchesConstraintJacobianFd) {
    ...
    for (int idx = DDP_IDX_V; idx <= DDP_IDX_OMEGA; ++idx) {
...
TEST_F(DdpEsdfConstraintTest, StrideSamplingSkipsIntermediateStages) {
    DdpEsdfConstraintConfig config;
    config.stride = 2;

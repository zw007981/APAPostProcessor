...
TEST(ApaEsdfMapAdapterTest, QueryDistanceMatchesUnderlyingEsdfMap) {
    ...
    const auto expected = esdf_map.getDistAndGrad(query_point.x(), query_point.y());
...
TEST(ApaEsdfMapAdapterTest, QueryDistanceConsistentAcrossMultiplePoints) {
    ...
    const std::vector<Eigen::Vector2d> query_points{
...
// 固定给出的(距离=0, 梯度=0)。触发原因：无解bug修复后（ESDF碰撞约束由硬约束改为软代价），
...
TEST(ApaEsdfMapAdapterTest, QueryDistanceClampsOutOfBoundsPointToMapInterior) {
    ...
    const auto raw = esdf_map.getDistAndGrad(out_of_bounds_point.x(), out_of_bounds_point.y());

...
class MincoThetaSManeuverSegmenterTestAccessor : public MincoThetaSManeuverSegmenter {
   public:
    using MincoThetaSManeuverSegmenter::MincoThetaSManeuverSegmenter;
    using MincoThetaSManeuverSegmenter::segmentManeuver;
};
...
// 在换挡点处连续、后退段沿负方向累积。
TEST(MincoThetaSManeuverSegmenterTest,
     ShiftSplitProducesCorrectDirectionsAndSignedArcLengths) {
    const MincoThetaSManeuverSegmenter segmenter(MakeConfig());
    const Path path = BuildTwoShiftPath();
    ASSERT_EQ(path.numManeuvers(), 3);

    const std::vector<MincoThetaSManeuverEstimate> estimates = segmenter.segment(path);
...
TEST(MincoThetaSManeuverSegmenterTest,
    ...
    const std::vector<MincoThetaSManeuverEstimate> tail_estimates =
        MincoThetaSManeuverSegmenter(config).segment(path);
...
TEST(MincoThetaSManeuverSegmenterTest,
    ...
    const std::vector<MincoThetaSManeuverEstimate> estimates =
        MincoThetaSManeuverSegmenter(MakeConfig()).segment(path);
...
// M=0。
TEST(MincoThetaSManeuverSegmenterTest, ShortManeuverDegeneratesToSingleSegment) {
    Path path;
    path.addPoint({0.0, 0.0, 0.0});
...
// 单段输出。
TEST(MincoThetaSManeuverSegmenterTest, SinglePointAndTwoPointPathsHaveDefinedBehavior) {
    const MincoThetaSManeuverSegmenter segmenter(MakeConfig());
    ...
    const std::vector<MincoThetaSManeuverEstimate> single_estimates =
    ...
    const std::vector<MincoThetaSManeuverEstimate> two_estimates =
...
TEST(MincoThetaSManeuverSegmenterTest, EmptyPathThrows) {
    const MincoThetaSManeuverSegmenter segmenter(MakeConfig());
...
// M=0 的未定义输出。
TEST(MincoThetaSManeuverSegmenterTest, EmptyManeuverThrows) {
    const MincoThetaSManeuverSegmenterTestAccessor segmenter(MincoThetaSConfig{});
...
TEST(MincoThetaSManeuverSegmenterTest, ThetaIsUnwrappedAcrossWrapBoundary) {
...
TEST(MincoThetaSManeuverSegmenterTest, InvalidConfigThrows) {
    const auto expect_throw = [](MincoThetaSConfig config) {
        EXPECT_THROW(MincoThetaSManeuverSegmenter{config}, std::invalid_argument);
    };
    expect_throw([] {
    ...
    EXPECT_NO_THROW(MincoThetaSManeuverSegmenter(MincoThetaSConfig{}));
}
...
void ExpectArcContinuity(const std::vector<MincoThetaSManeuverEstimate>& estimates) {
...
TEST(MincoThetaSManeuverSegmenterTest, FusionOffSwitchKeepsAllRuns) {
    const Path path = BuildWigglePath();
    ASSERT_EQ(path.numManeuvers(), 3);
    const auto estimates = MincoThetaSManeuverSegmenter(MakeConfig()).segment(path);
    ASSERT_EQ(estimates.size(), 3);
    EXPECT_EQ(estimates[1].direction, Direction::BACKWARD);
...
TEST(MincoThetaSManeuverSegmenterTest,
    ...
    const auto estimates =
        MincoThetaSManeuverSegmenter(config).segment(BuildWigglePath());
    ASSERT_EQ(estimates.size(), 1);
    EXPECT_EQ(estimates[0].direction, Direction::FORWARD);
...
TEST(MincoThetaSManeuverSegmenterTest, FusionProtectsFirstAndLastRuns) {
    // 末段微小：前进2.0m → 后退0.3m
    Path last_tiny;
    ...
    config.fuse_arc_threshold = 0.5;
    config.fuse_heading_threshold = 0.2;
    const MincoThetaSManeuverSegmenter segmenter(config);
...
TEST(MincoThetaSManeuverSegmenterTest, FusionSkipsLargeHeadingChangeRuns) {
    Path path;
    path.addPoint({0.0, 0.0, 0.0});
    AppendXLine(&path, 0.0, 1.0, 0.0);
    ...
    config.fuse_arc_threshold = 0.5;
    config.fuse_heading_threshold = 0.2;
    const auto estimates = MincoThetaSManeuverSegmenter(config).segment(path);
...
// 3.0m，弧长连续。
TEST(MincoThetaSManeuverSegmenterTest, FusionChainsAcrossMultipleWiggles) {
    Path path;
    path.addPoint({0.0, 0.0, 0.0});
    AppendXLine(&path, 0.0, 1.0, 0.0);
...
TEST(MincoThetaSManeuverSegmenterTest, PivotRunIsNeverFused) {
    Path path;
    path.addPoint({0.0, 0.0, 0.0});
    AppendXLine(&path, 0.0, 1.0, 0.0);

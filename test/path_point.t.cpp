...
// 测试 PathPoint 默认构造时所有派生量均未设置。
// 因为默认值统一用 NaN 表示"尚未提供"，所以几何量继承 Pose 的 0 初值，且五个 hasXxx() 都应返回 false。
...
// 测试使用 x、y、theta 构造 PathPoint 的场景。
// 因为几何量必须直接来自输入且派生量默认未提供，所以应同时初始化 x/y/theta 并保持五个 hasXxx() 为 false。
TEST(PathPointTest, ConstructWithValuesInitializesPoseAndLeavesDerivedUnset)
{
    const PathPoint point{1.2, 3.4, 0.6};
...
// 测试基于 Pose 构造 PathPoint 的场景。
// 因为 Pose 只携带 x/y/theta，所以转换后几何量应完整保留且所有派生量保持未设置状态。
TEST(PathPointTest, ConstructFromPoseInitializesPoseAndLeavesDerivedUnset)
{
    const Pose pose{2.1, 4.3, 0.7};
    const PathPoint point(pose);
...
// 测试 PathPoint 继承 Pose 的 theta 归一化行为。
// 因为构造时调用基类 Pose 构造函数，所以超过一圈的角度应折叠回主值域。
TEST(PathPointTest, ConstructNormalizesThetaLikePose)
{
    const PathPoint near_pi_point{0.0, 0.0, 3.14159};
    const PathPoint positive_wrap_point{0.0, 0.0, 3.0 * PI};
    const PathPoint negative_wrap_point{0.0, 0.0, -3.0 * PI};
...
{
    const PathPoint point{1.234, 5.678, 0.456};

    EXPECT_EQ(point.toString(),
              std::string("{\"x\": 1.23, \"y\": 5.68, "
                          "\"theta\": 0.46}"));
}

// 测试 toString 在设置部分派生量后的输出场景。
// 因为只有被 setXxx() 写入的字段才应被序列化，所以输出只包含 kappa/a 而不含 v/delta/delta_dot。
TEST(PathPointTest, ToStringIncludesOnlySetDerivedFields)
{
    PathPoint point{1.234, 5.678, 0.456};
...
// 测试 PathPoint 可以隐式向上转型为 const Pose&。
// 因为 PathPoint 是 public 继承 Pose，所以多态/切片场景下应保持兼容。
TEST(PathPointTest, ImplicitUcastToConstPoseReference)
{
    const PathPoint point{1.0, 2.0, 0.5};

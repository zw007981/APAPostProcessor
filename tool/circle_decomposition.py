...
class VehicleCircleDecomposition:
    ...
    def print_overhang_report(self):
        ...
        print("=" * 64)
        print(
            f"  车辆: {self.vehicle_length:.2f}×{self.vehicle_width:.2f} m, "
    ...
    def plot_decomposition(self, save_path: str = "circle_decomposition_plot.png"):
        ...
        for i, (cx, cy, r, _) in enumerate(self.outer_circles):
            label = (
                f"Outer ({len(self.outer_circles)} circles, NMPC ng)" if i == 0 else ""
            )
        ...
        for i, (cx, cy, r, _) in enumerate(self.inner_circles):
            label = (
                f"Inner ({len(self.inner_circles)} circles, validation)"
                if i == 0
                else ""
            )
            circle = patches.Circle(
        ...
        ax.set_aspect("equal")
        ax.set_xlim(
            -self.rear_overhang - margin,
            self.vehicle_length - self.rear_overhang + margin,
        )
        ax.set_ylim(-self.vehicle_width / 2 - margin, self.vehicle_width / 2 + margin)

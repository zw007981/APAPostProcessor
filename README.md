...
| **Quill** | `third_party/quill/` | 高性能异步日志库 |
...
基于NMPC思想优化HybridA*等方法生成的初始轨迹，这里提供了三个场景四个用例，在每个用例中的表现如下：

| 数据集 | 优化前后长度变化 | maneuver变化 | 耗时 | 收敛状态 |
|---|---|---|---|---|
| `data/long_park/data6.json` | 36.862→36.836m | 6→6 | 5.4s | 求解失败，回退到预处理轨迹 |
| `data/mid_park/data3.json` | 24.582→20.454m | 9→7 | 17.8s | 未完全收敛，使用末次迭代 |
| `data/rub_park/data1.json` | 12.988→10.554m | 10→8 | 11.2s | 未完全收敛，使用末次迭代 |
| `data/rub_park/data7.json` | 18.744→16.363m | 6→5 | 11.4s | 收敛 |
...
然而，这一自由度是有代价的：软约束仅在求解收敛的前提下才能保证无碰撞，一旦优化未能收敛，中间迭代轨迹便可能与障碍物发生碰撞，甚至卡死于障碍物内部。出于"未收敛情形下也必须保证安全"的工程考虑，我们主动舍弃了这一自由度，改为引入静态走廊硬约束，将优化空间尽可能限制在初始路径的同伦类内。其代价在本场景中同样直观：优化器无法再从障碍物右侧绕行，优化失败：

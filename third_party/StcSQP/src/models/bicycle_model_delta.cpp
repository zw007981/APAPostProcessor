#include "bicycle_model_delta.h"

namespace stc_SQP {
BicycleModelDelta::BicycleModelDelta(double wheelbase)
    : wheelbase_(wheelbase)
{
    if (wheelbase_ <= 0.0) {
        throw std::invalid_argument("BicycleModelDelta: wheelbase must be greater than 0");
    }
}

void BicycleModelDelta::evaluate(const Vector& x, const Vector& u, Vector& x_dot) const
{
    //   x = [x_pos, y_pos, theta, v, delta]
    //   u = [a, delta_dot]
    //   x_dot = [v*cos(theta), v*sin(theta), v*tan(delta)/L, a, delta_dot]
    x_dot.resize(NX);
    const double theta = x(IDX_THETA), v = x(IDX_V), delta = x(IDX_DELTA),
                 a = u(IDX_A), delta_dot = u(IDX_DELTA_DOT);
    const double c = std::cos(theta), s = std::sin(theta);
    x_dot(IDX_X) = v * c;
    x_dot(IDX_Y) = v * s;
    x_dot(IDX_THETA) = v * std::tan(delta) / wheelbase_;
    x_dot(IDX_V) = a;
    x_dot(IDX_DELTA) = delta_dot;
}

void BicycleModelDelta::computeContinuousJacobians(const Vector& x, Matrix& A_c,
    Matrix& B_c) const
{
    // 注意 theta 对 delta 的导数含 sec^2(delta)/L，是 delta 模型非线性的主要来源。
    A_c.setZero(NX, NX);
    B_c.setZero(NX, NU);
    const double theta = x(IDX_THETA), v = x(IDX_V), delta = x(IDX_DELTA);
    const double c = std::cos(theta), s = std::sin(theta);
    const double cos_delta = std::cos(delta), sec_delta_sq = 1.0 / (cos_delta * cos_delta);
    ...
    B_c(IDX_V, IDX_A) = 1.0;
    B_c(IDX_DELTA, IDX_DELTA_DOT) = 1.0;
}

void BicycleModelDelta::discretizeAndLinearize(const Vector& x, const Vector& u, double dt,
...
void BicycleModelDelta::retract(const Vector& x, double alpha, const Vector& delta,
    Vector& x_new) const
{

...
void MakeScenario(DdpReference* reference, DdpAlignedVec<DdpState>* states,
    ...
    reference->dt = kDt;
    reference->ds = 0.05;
    DdpState x = DdpState::Zero();
    x(DDP_IDX_V) = 0.02;
    states->push_back(x);
    reference->poses.emplace_back(x(DDP_IDX_X), x(DDP_IDX_Y), x(DDP_IDX_THETA));
    for (std::size_t k = 0; k < kHorizonSteps; ++k) {
        DdpControl u;
        // 前 200 步前进、后 199 步倒车，j/η 均按平滑伪机动剖面变化
        const double sign = k < 200 ? 1.0 : -1.0;
        u(DDP_IDX_JERK) = 0.15 * std::sin(0.05 * static_cast<double>(k)) * sign;
        u(DDP_IDX_ETA) = 0.25 * std::cos(0.04 * static_cast<double>(k));
        controls->push_back(u);
        x = dynamics.step(x, u, kDt);
        states->push_back(x);
        reference->poses.emplace_back(x(DDP_IDX_X), x(DDP_IDX_Y),
                                      x(DDP_IDX_THETA));
    }
}
...
DdpCostMultiplierState MakeMultipliers() {
    auto multipliers = DdpCostMultiplierState::MakeZero(kHorizonSteps);
...
void BM_DdpCostFullEvaluationWithEsdf(benchmark::State& state) {
    ...
    const DdpEsdfConstraint esdf_constraint(bundle.esdf_map,
...
BENCHMARK(BM_DdpCostFullEvaluationWithEsdf);

// 同场景不含 ESDF 的对照组：分离纯二次项装配成本与 ESDF 查询成本
// （设计文档预期运行时被 ESDF 查询主导）
void BM_DdpCostFullEvaluationWithoutEsdf(benchmark::State& state) {
    DdpReference reference;
    DdpAlignedVec<DdpState> states;
    DdpAlignedVec<DdpControl> controls;
    MakeScenario(&reference, &states, &controls);
...
BENCHMARK(BM_DdpCostFullEvaluationWithoutEsdf);
